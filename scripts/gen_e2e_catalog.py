...
def parse_row(src: str, name: str) -> Row | None:
    """The row in `src`'s module docstring, or None when it carries no block.
    ...
    if not any(_FIELD.match(ln) for ln in lines):
        return None
...
def collect(e2e_dir: Path) -> tuple[list[Row], list[str], list[str]]:
    ...
    for path in sorted(e2e_dir.glob("test_*.py")):
        try:
            row = parse_row(path.read_text(encoding="utf-8"), path.name)
        except CatalogError as exc:
            errors.append(str(exc))
            continue
        (rows.append(row) if row else pending.append(path.name))
...
def render(rows: list[Row], pending: list[str]) -> str:
    ...
    out: list[str] = [
        ...
        "",
        f"**{len(rows)} of {total} files catalogued.**",
        "",
    ]
    for row in rows:
        out += [
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.write:
        ...
        print(
            f"[e2e-catalog] wrote {CATALOG.relative_to(REPO_ROOT)} "
            f"({len(rows)}/{len(rows) + len(pending)} catalogued)"
        )
        return 0

    current = CATALOG.read_text(encoding="utf-8") if CATALOG.exists() else None
    if current == rendered:
        print(f"[e2e-catalog] current ({len(rows)}/{len(rows) + len(pending)} catalogued)")
        return 0
