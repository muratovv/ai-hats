...
_REASON_TEXT: dict[str, str] = {
    "git-dirty": "File had uncommitted changes; left intact so your in-flight "
    "diff isn't clobbered.",
    "unsupported-ext": "File extension not in the auto-heal allowlist "
    "(`.md` / `.txt` / `.j2` / `.sh` / `.envrc`).",
    "dst-missing": "**Possible data loss.** Neither the legacy source nor the new "
    ...
    "user-hook-disabled": "User-owned hook **disabled** in settings.json by the v4 "
    ...
    "auto-heal": "Auto-heal candidate that was not applied (mixed-state file).",
}
...
def write_inventory(layout: ProjectLayout, refs: list[LegacyRef]) -> Path | None:
    ...
    for file_path in sorted(by_file, key=lambda p: str(p)):
        ...
        for ref in sorted(by_file[file_path], key=lambda r: r.line):
            ...
            lines.append(
                f"- {loc} `{ref.legacy_substr}` → `{ref.new_substr}` _(reason: `{ref.reason}`)_"
            )
...
def _print_healed(file_path: Path, refs: list[LegacyRef], layout: ProjectLayout) -> None:
    ...
    for ref in refs:
        loc = f"L{ref.line}" if ref.line > 0 else "(json)"
        print(
            f"[heal] Healed: {rel} {loc} '{ref.legacy_substr}' → '{ref.new_substr}'",
            file=sys.stderr,
        )
...
def _print_summary(report: HealReport, layout: ProjectLayout) -> None:
    ...
    if healed_count:
        print(
            f"[heal] Auto-healed {healed_count} legacy-path ref(s) across user-managed files.",
            file=sys.stderr,
        )
    if report.inventory_path:
        ...
        if other:
            print(
                f"[heal] Manual fixes required for {other} other ref(s) — see {rel}",
                file=sys.stderr,
            )
