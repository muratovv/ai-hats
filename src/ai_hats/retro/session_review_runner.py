...
class SessionReviewRunner:
    ...
    def _build_prompt(self, facts) -> str:
        ...
        sections.append(
            ...
            "                     observation meets the HYP's success criterion\n"
            "      refuted      — the session shows the opposite: the baseline the HYP\n"
            "                     predicts away from persists, or got worse\n"
            "      inconclusive — the session has relevant data, but it is mixed,\n"
            "                     partial, or does not settle the question\n"
            "      n/a          — the session PHYSICALLY CANNOT test this HYP\n"
            "    Two bars that are easy to get wrong:\n"
            "      `n/a` is narrow. Merely being unsure whether the HYP applies is\n"
            "      `inconclusive`, never `n/a`.\n"
            "      Absence of the failure a guard prevents is NOT evidence the guard\n"
            "      worked — without the guard visibly firing, that is `inconclusive`,\n"
            "      never `confirmed`.\n"
            "  proposal_actions:\n"
