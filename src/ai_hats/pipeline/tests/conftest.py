...
— the same gap ``packages/*/tests`` has, answered the same way:
...
# resolves library layers at COLLECTION time. Both halves, never one: the pairing
...
def _isolate_ai_hats_dir(monkeypatch):
    """Mirrors the main suite's ``_isolate_ai_hats_dir`` fixture: an ambient ``AI_HATS_DIR``
    ...
    monkeypatch.delenv("AI_HATS_PROJECT_DIR", raising=False)  # pair var
    yield
