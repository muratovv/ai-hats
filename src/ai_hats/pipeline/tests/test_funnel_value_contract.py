...
D3 in ADR-0005: the pipeline funnel value contract says
...
trap that leaked a None-valued key into context physically unreachable for any current or
future step author.
...
def test_funnel_drops_none_values_from_step_delta(tmp_path):
    ...
    assert "absent_key" not in final, (
        "None-valued key leaked into context (should be dropped at funnel merge per D3)"
    )
