...
def attach_add(task_id: str, path: Path, name: str, note: str):
    ...
    console.print(
        f"[green]{verb}[/]: '{attach_name}' (digest {result.attachment.digest}) "
        f"to {task_id}"
    )
...
def attach_show(task_id: str, name: str):
    ...
    if entry is None:
        console.print(f"[red]Error[/]: '{name}' not attached to {task_id}")
        sys.exit(1)
...
def attach_remove(task_id: str, name: str, yes: bool):
    ...
    entry = next((a for a in task.attachments if a.name == name), None)
    if entry is None:
        console.print(f"[yellow]Noop[/]: '{name}' not attached to {task_id}")
        return
    ...
    if removed is None:
        console.print(f"[yellow]Noop[/]: '{name}' was not in manifest")
        return

    if tracked:
        console.print(
            f"[green]Removed[/]: '{name}' (git-tracked — restorable via "
            "[bold]git restore[/])"
        )
    else:
        console.print(f"[green]Removed[/]: '{name}' (permanent)")
