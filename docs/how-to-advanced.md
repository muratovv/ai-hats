...
| `requires` | Keys that **must** be in pipeline state when the step runs. Missing → `BuildError` at validation, before anything fires. This list is also what `run` is called with, so every parameter `run` has no default for must appear here — `build()` refuses a step whose signature demands a key it never declared, because the projection could never pass it. |
| `optional` | Keys the step may consume if present; missing ones are silently absent.                                                                                                                                                                                                                                                                                    |
| `produces` | Keys this step is allowed to emit. Returning anything outside this set raises `StepError` and aborts the pipeline. Declaring a key an earlier step also produces is legal only if something between them reads it — otherwise `build()` raises `BuildError`, because the overwrite would lose that value silently.                                         |
...
| ------------------------------------------------ | ----------------------------------------------------------------------------------------- |
| Non-trivial change you want isolated from master | yes — `ai-hats wt create` (or just `rack transition <id> execute`, which does it for you) |
...
```bash
# Create (or — typically — let `rack transition <id> execute` create it for you)
...
`rack transition <id> execute` opens a worktree for the task and the agent works there. To delegate a sub-task to a separate agent in its own isolated worktree:
...
| Two parallel `rack transition <id> done` on tasks sharing the **same base** ref | Serialized at the base-branch merge layer; both DONE, both commits land. On a real merge failure the task **stays in `review`** instead of silently going DONE (fail-loud). |
...
- [2] — `rack transition <id> execute` and `rack transition <id> done` lifecycle (the most common worktree entry/exit points).
