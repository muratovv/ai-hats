...
- **Don't `cp` skill files manually.** At runtime, ai-hats materializes the role's skills into a per-session plugin-dir (`<ai_hats_dir>/.cache/sessions/<sid>/plugin/`) and passes it to `claude` via `--plugin-dir` — edits to skills take effect automatically on the next session without running any command. There is no permanent `.claude/skills/` or `.agent/ai-hats/library/skills/` skill-mirror to maintain since HATS-294/1165. **Never** `cp -r .claude/skills/ ~/.claude/skills/`: ai-hats does not manage user-level Claude skill catalogs, the copy will drift from source-of-truth, and `self init` will print a WARN about the orphan `.ai-hats-managed` marker on every run (HATS-465).
