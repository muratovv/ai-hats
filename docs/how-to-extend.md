...
- **`usage/`** — curated content catalog. Opinionated roles (`assistant`, `dev-python`, `dev-web`, `architect`, `sre`, `go-dev`, `go-dev-full`, `tech-writer`), domain traits (`trait-se-mindset`, `dev::python`, `dev::web`, `dev::shell`, `dev::go-*`, `env::proxmox`, …), and the optional skills (golang, terraform, ansible, observability, system-design, component authoring, …).
