...
- **`usage/`** — curated content catalog. Opinionated roles (`assistant`, `dev-python`, `dev-web`, `behaviorist`, `architect`, `sre`, `go-dev`), domain traits (`trait-se-mindset`, `dev::python`, `dev::web`, `dev::shell`, `dev::go-*`, `env::proxmox`, …), and the optional skills (golang, terraform, ansible, observability, system-design, component authoring, …).
