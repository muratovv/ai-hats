...
Verification: after `ai-hats self init` the skill is added to the role composition and available in session executions.
...
| Override venv broken                                                                      | `uv venv --python 3.13 <override-path> && uv pip install --python <override-path>/bin/python 'ai-hats @ git+https://github.com/muratovv/ai-hats.git'` (user-managed) |
| `self update` fails with `does not satisfy Python>=3.13`                                  | `curl -LsSf https://github.com/muratovv/ai-hats/raw/master/scripts/bootstrap.sh \| bash -s -- --repair` (one-time, crossing the Python pin bump — see below)         |
| Full project wipe (data loss!)                                                            | `rm -rf .agent/ai-hats/ && ai-hats self update && ai-hats self init -r <role> -p <provider>`                                                                         |

**Crossing the 3.13 pin bump, once.** An install made before the bump runs a Python that pins 3.11, and `self update` builds the new version's venv with **its own** pin before installing into it — so the new distribution, which requires `>=3.13`, cannot be resolved and the update exits 1 with uv's `does not satisfy Python>=3.13`. Nothing is damaged: the old install keeps working. `self update` cannot fix this from the inside (the code deciding the interpreter is the old code), which is exactly the bootstrap paradox `--repair` exists for: it reinstalls the launcher and removes the managed venv, and the **launcher** — not the old Python — then builds the replacement on 3.13. One command, once per install.
...
**[5]** — [`docs/reflect.md`](reflect.md) — retrospective pipeline architecture, by component: `session-reviewer` (per-session LLM auditor), `judge-auditor`/`judge` (HYP + PROP triage), `role-judge` (role audit), `hypothesis-intake` (observation intake).
