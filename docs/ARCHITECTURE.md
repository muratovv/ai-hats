...
- **Task (`HATS-NNN`)** — a unit of planned work. Happy path — the fixed pipeline `brainstorm → plan → execute → document → review → done` without skipping states. Side routes: `blocked` (returnable to `plan` or `execute`), `failed` (recoverable via `brainstorm`), `cancelled` (administrative close from any non-terminal state), from `review` a rework path back to `execute` for addressing review comments (no worktree merge, unlike `review → done`), and from `done` a reopen path to `execute` is available for finishing epic scope. Shortcut: `rack transition <id> --state done --force --reason "..."` fast-closes a `brainstorm`/`plan` task straight to `done` when the work shipped on master (no worktree theatre). `--force` relaxes the FSM arrow **only**, requires a reason, and journals it — it does **not** relax the `->done` gate either, which since HATS-1719 is bound to EVERY road into `done` rather than to `review->done` alone: the fast-close merges into master like any other road, so it is asked the same question (*is master green after this card*). Nor does it relax consent: consent is a property of the move, not of the command, so nothing added to the command line switches it off (`consent | op --force`, HATS-1682). A forced close still works, and since HATS-1752 it is asked the same question as any other road into `done` — how often depends on the grant window, never on the command. Cross-references between cards are typed links — `parent_task`, `depends_on`, `related`, `see_also`, the directional `folded_into` (this card was subsumed by that one), and the derived `children` and `blocks` (the read-only reverse of `parent_task` and `depends_on`) — managed via `rack transition <id> --link <kind>:<id>`; an unknown kind is a typed refusal listing the legal set. On the transition to `plan` a `plan.md` scaffold is created. Which edges are **consent-gated** is the ROLE's declaration, not the backlog's: a `composition.apps` row carrying `consent: true` names the points a role wants the supervisor asked on — a row carries `run:`, `consent:`, or both — and the agent trait declares `plan->execute` and `->done` for `rack`, plus `pre-merge` for `wt`: the two roads into master. `->done` is every road into the state, the forced close included (HATS-1752) — it was `review->done` only while a wide question cost a click per move, and the grant window (HATS-1735) retired that cost. `plan->execute` stays an exact edge on purpose: a wide `->execute` would gate the rework loop, which lands nothing on master. On a declared point the guard raises a permission prompt in chat, whose approval mints a one-shot ticket bound to that call (`AI_HATS_CONSENT_TICKET`); the engine refuses the move without one. It falls back to `AI_HATS_CONSENT_ACK=1` in the launching environment wherever no question can be asked (headless, cron, a surface without runtime hooks) — one flag for the whole move, including the worktree merge inside `review → done`, and never silent: every gate it opens writes a `hatch` line to the bypass journal, and the two inside the transition (the edge gate and the teardown merge) also record it in the card's work log. It is the launching environment's to export; typed inline it is refused as a self-grant. `AI_HATS_PLAN_ACK=1` still answers `plan → execute` alone; `AI_HATS_MERGE_ACK=1` answers a direct `ai-hats wt merge` and deliberately **not** `review → done`. The work log is written with session tracking; a file lock protects against race conditions.
