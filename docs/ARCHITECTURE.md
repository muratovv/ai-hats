...
```mermaid
flowchart LR
  User([User]) -->|ai-hats agent ROLE| Launch[Launch session]
  Launch -->|session_start hook| Interactive[Interactive session]
  Interactive -->|every turn| Trace[(Trace log)]
  Interactive -->|on close| Finalize[Finalize artifacts]
  Finalize --> Artifacts[(audit + metrics +<br/>transcript)]
  Finalize -->|session_end hook| Retro{Auto retro?<br/>policy + threshold}
  Retro -->|off / under threshold| Done([end])
  Retro -->|run| Bridge[/Continue in<br/>Reflection loop/]
```
