...
session end
  ...
  reflect_session_main.run_session_review(sid, max_retries, project_dir)
    ...
    │    Runs before either error branch below
...
### `ai-hats reflect hypothesis` (ADR-0007)
...
| `hats-reflect-session` | `ReflectSessionV1` | historical — read-only     |
