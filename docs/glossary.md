...
- **`compute_usage` step** — `ComputeUsage` (file `src/ai_hats/pipeline/steps/compute_usage.py`, HATS-664). Sibling of `make_audit`: same post-session JSONL source (`~/.claude/projects/<key>/<claude_session_id>.jsonl`, same `_discover_claude_jsonl` fallback) but emits a SEPARATE machine-readable `<session_dir>/usage.json` (`usage/v1`) instead of the human `audit.md`. The report carries measured always-on (first `cache_creation` proxy), an ordered event timeline (skill-body loads, reference Reads, tool calls, stop-hook firings), aggregates with tool success-rate, and sidechain linkage. Heavy lifting is the pure `parse_session_usage` in `src/ai_hats/usage.py` (transcript-only, fail-soft, also runnable as `python -m ai_hats.usage <jsonl>` for retroactive sweeps); the step is the thin live driver that resolves the JSONL, optionally enriches always-on with a static `costs.py` cross-check when `role` is in the funnel, and persists. `failure_policy = "continue"`. Used in BOTH `finalize-hitl` and `finalize-subagent`, positioned right after `make_audit`. Token attribution per event is a documented `reconstructed` heuristic (per-message usage is a per-turn total) — unattributable events keep `tokens_delta = null`, never a magic `0`.
