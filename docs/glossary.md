...
- **Versioned install layout** — `<ai_hats_dir>/versions/<sha>/` (a per-`sha` venv) + a `versions/current` pointer-file (HATS-647). A managed `ai-hats self update` installs the new version into a fresh `versions/<sha>/` and atomically flips `current` (tmp-write + `os.replace`) only after a successful install + verify — so an update never mutates the venv a live run executes from, and an interrupted update never bricks the tool (`current` always points at a complete venv; on crash it stays on the old `sha`). The bash launcher resolves `current → versions/<sha>` once at run start and **exports `AI_HATS_VENV`**, pinning the whole run (hooks, sub-agents, bash-chained `ai-hats … | ai-hats …`) to one `sha` even if `current` flips mid-run (pin-at-spawn). Applies only to the managed default venv; editable / dev installs and explicit `AI_HATS_VENV` / yaml `venv_path` overrides bypass versioning, and a pre-versioning `.venv` is the lazy fallback. GC of superseded versions is R2 (HATS-649); crash-safe `.tmp-<sha>` staging + `.complete` sentinel + recovery sweep is R1 (HATS-648).
