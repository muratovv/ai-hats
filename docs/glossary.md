...
- **Recovery (both guards).** `git checkout <expected>` in the main repo, then retry `wt merge` / `task transition done`. The worktree branch is preserved across the refusal — no work lost. Both CLI surfaces (direct `wt merge` and `task transition done`) emit a copy-pasteable recipe.
