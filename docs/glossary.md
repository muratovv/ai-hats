...
- **Self-heal hooks** — `git-mastery` ships `self-heal-hooks.sh` as both **post-merge** and **post-checkout**; they call `sync-hooks` at the moment merge / pull / branch-checkout introduces drift. `WrapRunner` runs the same verb directly at session start for every role (`_resync_git_hooks`, HATS-707 — re-homed from the dead `session_start` lifecycle-hook channel; in-session safety net covering rebase / reset / manual drift the post-merge/post-checkout layer misses). All callers are **fail-open** (always `exit 0`, loud warning on failure); the dispatcher backstop is the one deliberately fail-closed surface. **Bootstrap caveat:** a fresh clone needs one initial `ai-hats self init` to install hooks at all — they self-maintain after that.
