...
The name (or names) of the branch that worktrees are expected to be created from and merged back into. Defaults to `master` and `main`, in that priority order; the first one that actually exists in the repo is the comparison target. A project can override the split (base ≠ merge-target) via the `worktree` block in `ai-hats.yaml` — see [How to configure › the `worktree` block](../how-to-configure.md#1a-the-worktree-block--fork-workflows-base--merge-target).
...
- **Why it matters.** `WorktreeManager.create()` captures whatever branch the main repo's HEAD currently points at as the worktree's `_original_branch` — and that is the branch `ai-hats wt merge` later lands commits on. Two silent-wrong-branch failure modes:
  - *Create-time.* Operator parks the main repo on a feature branch before `wt create` / `rack transition <ID> execute`. The worktree quietly inherits that branch as its merge target; CLI reports "merged" while master never sees the work.
...
- **Two refusal gates.**
  - *Create-time refusal* — no worktree exists yet; the refusal aborts before `git worktree add` runs. Retry creates the worktree fresh once HEAD is on a canonical base.
  - *Merge-time refusal* — the worktree dir and worktree branch are preserved untouched; the refusal happens before `_check_clean` / `_check_drift` / the actual `git merge`. Retry from the corrected HEAD finishes the merge as if the refusal hadn't happened.
- **`--force` / `--accept-drift` do NOT bypass either guard.** `--force` is the dirty-worktree consent; `--accept-drift` is the stale-baseline consent (drift itself is cleared by rebasing). Neither addresses wrong-branch protection — three independent safety contracts, three independent flags.
- **Configurable per project.** The default is the `master`/`main` two-name set, but a fork/dogfood repo can point the split (base ≠ merge-target) at its own branches via the `worktree` block — full contract in [How to configure](../how-to-configure.md#1a-the-worktree-block--fork-workflows-base--merge-target). When unset, behavior is byte-identical to the historical hardcoded set.

## Worktree checkout root

The directory under which `ai-hats wt create` mints linked worktrees — resolved by **`worktree_checkouts_dir(project_dir)`** to `<cache_root>/worktrees/`, one subtree per project key. **Not to be confused with `worktrees_dir(project_dir)`**, which is the per-task worktree **metadata** (state JSON + the L1/L2 lock files) under `<ai_hats_dir>/sessions/worktrees/`. Trees on one side, bookkeeping about them on the other; the names are deliberately different words.

Injected, not imported: ai-hats passes the root at construction the same way it passes `state_dir` (ADR-0013 D4), so the engine never imports `ai_hats.paths`. A bare-core consumer that injects nothing keeps the historical `tempfile.mkdtemp` fallback (D9).

**Why not `$TMPDIR`.** macOS reaps the per-user temp root by *access* time — `com.apple.bsd.dirhelper` runs daily at 03:35 with `CLEAN_FILES_OLDER_THAN_DAYS=3` — deleting files and leaving the directory skeleton. A worktree venv is born already expired, because `uv` materializes packages from `~/.cache/uv` preserving the cache's atime: measured on a venv hours old, 595 `.py` files carried an atime 15 days stale. The cold half of the environment then vanishes at the next sweep (measured: 542–554 of 2234 `.py` survived, against 574 predicted by the 3-day rule), and a worktree left idle past the threshold loses its **source tree** too (measured: 0 files, directories intact). The cache root is swept by nothing but ai-hats itself, and `_sweep_orphan_project_keys` spares a key whose worktrees are still alive.

## Worktree data transfer (carry-in / carry-out)

The mechanism by which **gitignored** data crosses the `ai-hats wt` boundary in either direction — **carry-in** (seeded *into* a worktree at `wt create`) or **carry-out** (harvested *out of* a worktree before `wt merge` / `wt discard` teardown). The primitive is a **worktree lifecycle hook** — a component-declared script run at `wt_in` (after create) or `wt_out` (before every teardown route, fail-closed). Shipped today is the `wt_in` / `wt_out` hook form; the declarative `seed_in` / `harvest_out` path-list sugar (designed in ADR-0012) is **shelved** — cancelled 2026-06-25, no confirmed consumer the primitive does not already serve.
