...
| 4    | **Customization (optional).** Shows the role's composition (`ai-hats config status`). On a Go project runs `ai-hats config suggest-traits`, which reads `go.mod` and prints a ready `config customize go-dev --remove-trait …` line for every Go domain the module requires nothing from. Offers to add / remove traits, rules, skills via `ai-hats config customize …`.                                                                                                      |
