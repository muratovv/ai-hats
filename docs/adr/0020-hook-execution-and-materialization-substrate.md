...
HATS-1338 (agy root + мерж `$HOME`), HATS-1339 (GC). Полная карта
...
- **Одна общая env-база — семь ключей.** Перечисление ниже **завершает**
  ...
  **Remove-if-None — свойство контракта, а не деталь реализации.**
  ...
  которой точка сработала (семь других ключей, приоритет и политика доверия
  чужому пину), — вторая половина env-контракта, и она живёт в ADR-0025 [6].
...
- [6] `docs/adr/0025-session-identity-env-contract.md` — вторая половина
  env-контракта: семь ключей идентичности сессии, роль `AI_HATS_PROJECT_DIR`,
