...
Дополнение 2026-09-05 (HATS-1883): `_scoped_override` и `venv_path` из
`paths/_dirs.py` ушли вместе с производными листа; процедура доверия — одна,
`ai_hats_core.layout.pin_is_foreign`, и её читают `ProjectLayout.compute` и
`Project.venv` (`src/ai_hats/cli/_entry.py`).
...
| `AI_HATS_DIR`               | база фреймворка этой сессии                                                                        | `get_env` каждого провайдера поверхности — `ClaudeProvider` (`src/ai_hats/surfaces/claude/provider.py`), `AgyProvider` (`src/ai_hats/surfaces/agy/provider.py`), `ClineProvider._env` (`src/ai_hats/surfaces/cline/provider.py`); все три пишут его вместе с пином |
...
| `AI_HATS_SESSION_CACHE_DIR` | резолвнутая директория сессионного кэша поверхности                                                | `AgyProvider._deliver_hooks` (`src/ai_hats/surfaces/agy/provider.py`)                                                                                                                                                                                              |
...
| `AI_HATS_PYTHON`            | интерпретатор процесса сессии — **второй** пин, потому что глобальный хук agy зовёт не наш лаунчер | `AgyProvider.get_env` (`src/ai_hats/surfaces/agy/provider.py`); читает `DISPATCHER_COMMAND` (`agy/global_hook.py`)                                                                                                                                                 |
...
**Конверт живой сессии — отдельная ветка.** Под `AI_HATS_SESSION_IDENTITY`
корень берётся из конверта без структурного обхода (инвариант ADR-0024:
потребитель идентичность не выводит), и таблица выше к нему не применяется:
чужой cwd не переадресует ответ. Одно исключение — **писатель**
(`resolve_project(writes=True)` в `src/ai_hats/cli/_entry.py`: `self init`,
`self bump`, `self update`, `config set`, `config customize`,
`config feedback`), стоящий внутри *другого* онбордед-проекта. Он отказывает
(`ForeignCwdError`), называя оба корня и форму обхода —
`env -u AI_HATS_SESSION_IDENTITY -u AI_HATS_PROJECT_DIR ai-hats …` — потому что
молчаливая запись в проект сессии из-под чужого cwd однажды переписала
`default_role` не того `ai-hats.yaml`. Читатель и хук под чужим cwd по-прежнему
отвечают конвертом: хук наследует cwd агента, и предупреждение там было бы шумом
на каждый вызов.
...
`src/ai_hats/surfaces/agy/provider.py`, `ClineProvider._env` в
`src/ai_hats/surfaces/cline/provider.py`), и каждая пишет ключ
...
- **C** — санкционированные зеркала, которым дом импортировать нельзя, несут те
  ...
  `ai_hats_rack.resolver` и `ai_hats_observe.trace`. Пятым был диспетчер хуков
  agy — он исполняется на **каждом** вызове инструмента и не имел права тянуть
  ai-hats: зеркало ради доступности, а не ради вкуса. HATS-1826 сложил
  поверхность внутрь интегратора, и зеркалу не осталось смысла:
  `ai_hats.surfaces.agy.hook_dispatcher` берёт спеллинги из дома напрямую
  (`ai_hats.env`, `ai_hats.session_identity`). Инвариант тот же — сверяется
  поведение, а не число копий;
