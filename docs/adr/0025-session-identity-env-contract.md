...
который потребляет слайс 1 оси [2] (ADR-0023 D6, «разрешение проекта и
...
| Читатель                                                   | Условие hop'а                                                                |
| ---------------------------------------------------------- | ---------------------------------------------------------------------------- |
| `scripts/ai-hats-launcher:28`                              | прыгает, **только если** в главном чекауте есть `.agent/` или `ai-hats.yaml` |
| `src/ai_hats/cli/_helpers.py:313-321`                      | прыгает безусловно (`WorktreeManager.main_worktree_root`)                    |
| `packages/ai-hats-rack/src/ai_hats_rack/resolver.py:68-87` | прыгает безусловно, чисто по ФС                                              |

Следствие: в linked worktree, чей главный чекаут **не** онбординжен, лаунчер
остаётся в worktree, а Python и rack уходят в главный — два разных корня на один
`cwd`. Это расхождение резолвера, а не политики доверия, и владеет им HATS-1606
(единый резолвер, слайс 1 оси [2]). Здесь оно (а) записано, чтобы восьмой автор
не выводил hop заново, и (б) заведено в матрицу конформанс-теста как
**наблюдаемое** — контракт, объявивший hop частью определения, не имеет права
оставлять его непроверенным.
...
обязательство ADR-0023 [2] формулирует как «сверять поведение, а не устранять
дублирование».
...
- **Тип-носитель `SessionEnv` / `HookEnv`.** Отвергнут: это пятая узкая копия
  значения проекта — ровно та болезнь, которую ADR-0023 [2] называет причиной
...
- [2] `docs/adr/0023-capability-ownership-and-the-project-value.md` — ось
