...
ревизия 2026-09-13 по ревью супервизора: файловый нейминг, тип `override`,
`EXECUTABLE` снят, конфиг без состава). Часть серии плоской модели: обзор и
слои — ADR-0033 [1]; правила, по которым этот DSL превращается в план, —
ADR-0035 [2]; форма плана и интерфейс харнеса — ADR-0036 [3]. Не реализован:
DSL и процессинг строятся этапом 1 (HATS-1976).
...
**Корни.** Корень не помечен на диске — его называет цепочка, и цепочку
строит код (`build_library_paths`, `src/ai_hats/library_paths.py`); с уходом
типовых каталогов исчезает единственный дисковый маркер, и ничто его не
заменяет. Владельцев у корней три; порядок last-wins идёт по цепочке, чистота
имён — по владельцу [2] D2:

| Владелец     | Корни, снизу вверх                                                                | Как устанавливается                                                                                                                                                             | Копия для `-re` (**D6**)                                                                                                             |
| ------------ | --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| поставляемое | `core`, `usage`, `ai-hats-dev`; пакеты                                            | `builtin_library_root`: env `AI_HATS_LIBRARY_ROOT` → исходный чекаут (только чтение) → установленный пакет; пакеты — entry-point `ai_hats.skills`                               | `<builtin_root>/experimental/{core,usage,ai-hats-dev}` — **рядом** со слоями, общий `hooks/` той же глубины; `<пакет>/experimental/` |
| пользователь | `~/.ai-hats/library/`; пути `library.paths` из `~/.ai-hats/config.yaml`           | каталог существует; путь назван                                                                                                                                                 | `~/.ai-hats/experimental/`; `<корень>/experimental/`                                                                                 |
| проект       | пути `library.paths` из `ai-hats.yaml`; `<project>/libraries/`; `--library <dir>` | относительно `<project>`, который берётся из конверта сессии, а без неё — обходом вверх от cwd; в linked worktree `project_dir` — MAIN, а `libraries/` перенаправлен в worktree | `<корень>/experimental/`                                                                                                             |

Файл принадлежит ровно одному корню: совпадающие пути в цепочке дедупятся,
вложенные — отказ с обоими путями, иначе у файла два имени. `.yaml` и `.md`
без `type` внутри корня — payload (`README.md`, `manifest.yaml`), с
предупреждением вне папки скилла [2] D8: опечатка в `type` не должна молча
превращать хук в payload. `~/.ai-hats/` после переезда — только конфиг;
библиотека пользователя — `~/.ai-hats/library/`, симметрично проектному
`libraries/`. Правило 2 границу payload не двигает: она — свой корень из этой
таблицы, для проекта — `libraries/`, не проект.
...
Пользовательский корень — `~/.ai-hats/library/` (на PoC —
`~/.ai-hats/experimental/`), конфиг рядом с ним его не касается; что
версионировать — кладётся в корень из `library.paths`. Оба — один владелец:
override в `~/.ai-hats/library/` вправе называть `personal-workflow` из
`~/dev/ai-hats-custom`, хотя тот выше по цепочке, — чистота имён идёт по
владельцу, а не по позиции [2] D2.
...
Новая форма живёт в каталоге `experimental/` у **каждого корня** цепочки —
колонка «Копия для `-re`» таблицы корней (**D1**). У поставляемой библиотеки
он один на три слоя и лежит **рядом** с ними, `<builtin_root>/experimental/{core,usage,ai-hats-dev}`:
общий payload `experimental/hooks/` тогда стоит на той же глубине от скриптов,
что сегодняшний `hooks/`, и 22 скрипта, ходящие в него относительным путём,
не правятся. У пользователя — `~/.ai-hats/experimental/`, у проекта —
`libraries/experimental/`, у корня из `library.paths` и пакета —
`<корень>/experimental/`. Копию создаёт **скрипт миграции**, не рука;
кандидат `-re` читает копии, эталон `-r` — сами корни, и ни один не мешает
другому. Условие готовности формата буквально — для каждого корня снять
старые каталоги и поставить копию на место:

```bash
# поставляемые слои
rm -rf <builtin_root>/{core,usage,ai-hats-dev}/{roles,traits,rules,skills}
for l in core usage ai-hats-dev; do mv <builtin_root>/experimental/$l/* <builtin_root>/$l/; done
# пользователь: библиотека отделяется от конфига
rm -rf ~/.ai-hats/{roles,traits,rules,skills}; mv ~/.ai-hats/experimental ~/.ai-hats/library
# проект и корни из library.paths
