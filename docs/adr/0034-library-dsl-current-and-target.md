...
- Ведущий `+` необязателен и ничего не значит: `-re "+ A"` ≡ `-re "A"`.
- Ведущий `-` законен: в `override` выражение применяется к выбранному
  композиту, в CLI — к `active` (**D4**).
...
```yaml
# done-gate.yaml — workflow: точка машины состояний приложения
schema_version: 1
type: hook
kind: workflow
app: rack
...
at: '->done'
run: done-gate.sh                    # payload: относительный путь от этого файла, внутри корня
on_error: refuse
```

Хук — вид, адрес точки, эффект, правило отказа; интерпретатор payload — по
shebang. Тождество для join — `(kind, адрес, run)`, где `run` — путь payload
от корня слоя [2] D6.
...
относительному пути; путь наружу корня — отказ. Соглашения миграции —
`hooks/<строка apps>.yaml` для строк `composition.apps`;
`hooks/<скилл>/<стем скрипта>.yaml` для каналов frontmatter (payload остаётся
в папке скилла, `run: ../../skills/<скилл>/hooks/<файл>` — скилл вправе звать
тот же скрипт из своего текста; имя по событию не годится: у `git-mastery`
четыре скрипта на `pre-commit`, у `safety-guard` четыре на `PreToolUse`);
...
```yaml
# rack.transition.yaml
schema_version: 1
type: operation
binaries: [rack, ai-hats]            # какие бинари произносят операцию
reader: rack_transition              # адаптер, читающий её из argv — код
subject: transition
```
...
Новую операцию без кода объявить нельзя, и это названо.
...
корней; верхний слой вправе переопределить его тем же именем; override'ы всех
корней дописывают ему своё по меткам в порядке `(слой, путь)`; версионируемый
конфиг называет `library.default` — выражение свежего клона; машинный —
`active`, выражение этой машины; CLI — последнее слово [2] D3. **Конфиг несёт
...
именем), `--dry-run` как флаг сессии
(его заменяет `dry-run` как проекция), `library_paths.yaml` и
`customizations.yaml` (их заменяет `~/.ai-hats/config.yaml` и корень
библиотеки). Безымянное выражение имени не получает: его идентичность —
каноническая строка выражения, и она едет в артефакты сессии как сегодняшний
`role_expression`.
...
| `composition.apps.rack.*` / `apps.wt.*` строка        | `hooks/<имя>.yaml`, `kind: workflow`; `run: <скилл>/<путь>` → относительный путь                                                                |
...
- Одна грамматика вместо семи каналов привязки и двух нотаций состава; три
