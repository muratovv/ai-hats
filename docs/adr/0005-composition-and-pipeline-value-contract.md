...
- `packages/ai-hats-library/src/ai_hats_library/ai-hats-dev/rules/rule_composition_value_contract/` — agent-facing reminder. Attached to `trait-agent` when this ADR was written, so it materialized in *every* agent session prompt; HATS-1834 moved it onto the `ai-hats-maintainer` trait, since the classes it names are integrator internals that 9 unrelated roles were paying always-on for.
