# ADR-0024: Env-контракт идентичности сессии — один набор ключей, одна процедура доверия
...
| `src/ai_hats/templates/githooks/dispatcher.sh:32-37`         | префиксная: `AI_HATS_DIR` принят, если лежит под `$PROJECT_DIR/`           | ignore молча                        |
...
Четыре из семи ведут себя правильно, три — нет: `_dirs.py:462-464`,
`dispatcher.sh:28-30` (одно и то же сырое чтение `AI_HATS_VENV` на двух языках) и
префиксный тест `dispatcher.sh:32-37`. Правильность при этом нигде не выведена из
...
модули интегратора их **ре-экспортируют**, а не объявляют заново. Исключение
одно и вынужденное: `AI_HATS_SESSION_ID` объявлен в `ai_hats_observe.trace`
...
Произведённый ai-hats `AI_HATS_DIR` всегда едет **вместе** с пином
(`scripts/ai-hats-launcher:155-159`, `src/ai_hats/surfaces/claude/provider.py:394-397`,
`src/ai_hats/hook_exec.py:215`), поэтому непарный ключ выставлен не фреймворком.
...
**Послабление, названное вслух.** Замена префиксного теста
`src/ai_hats/templates/githooks/dispatcher.sh:32-37` парным делает голый
`AI_HATS_DIR` вне `$PROJECT_DIR` — сегодня отвергаемый — honoured. Обоснование в
D3: непарный ключ выставлен человеком. Политик доверия внутри заглушки при этом
становится одна вместо двух.
