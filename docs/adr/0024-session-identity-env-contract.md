...
| `src/ai_hats/paths/library.py:111-118`                       | нет процедуры: пин взят как корень поиска библиотеки                       | honour                              |
...
ключа. Сегодня роль двойная и несовместимая: `paths/_dirs.py:154` и
`scripts/ai-hats-launcher:84` используют его **только** как область видимости
(корень резолвится независимо), а `paths/library.py:114` берёт его **как
корень**.

Правило одно, и оно разводит эти два прочтения по признаку читателя:

- Читатель, у которого **нет** структурного знания о своём проекте, **берёт
  пин**: это единственное имеющееся заявление о принадлежности.
- Читатель, который свой проект знает **структурно**, использует своё, а
  расходящийся пин считает **чужим** — принадлежащим не этой сессии.

*Структурное знание* — корень, выведенный из физического положения самого
читателя, а не из окружения. Все пять живых читателей его имеют:
...
то, чем он вычисляется. `unverified` остаётся явно помеченной слепой зоной — её
никогда не подменяет молчание.
