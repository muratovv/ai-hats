...
- **`[injections]`** — тексты инжекций трейтов, роли и оверлеев, склеенные
  ...
  «говорит» агенту.
- **`[rules]`** — резолвленные правила `(имя, source_path)`. Каждое скомпозированное правило доставляет своё тело в секцию `## RULES` промпта (HATS-1515). Правило — один файл `rule.md`; сайдкара `metadata.yaml` у него нет, а оставшийся от внешней библиотеки не читается (HATS-1836). Pre-commit гейт rule-delivery (`rule_delivery.py`) запрещает инжекции указывать на несуществующее правило.
...
**Почему поле `delivery` осталось — и почему потом ушло.** На момент HATS-1515
единственная его работа была предупредить о незнакомом значении: без него
`delivery: summarized`, написанный по старой документации, был бы проглочен
молча (`extra="ignore"` в `YamlModel.model_config`,
`ai_hats_core/yaml_model.py`) — ровно тот класс тишины, который та итерация и
закрывала.

**Отменено HATS-1836.** Tripwire держался на двух опорах: что файл существует и
что документация его описывает. HATS-1836 убрал у правил `metadata.yaml`
целиком — вместе с восемью носителями `delivery` и с документацией, из которой
поле можно было переписать, — так что стеречь стало нечего. Причина сноса не в
`delivery`, а в `description`: он держал вторую копию смысла правила, и 5 из 14
описаний уже разошлись со своими телами. Расхождение двух копий не проверяемо
гейтом, поэтому копия удалена, а `ai-hats list rules` печатает одни имена.
...
- **M10 — Поведение при отказе следует таксономии.** `in_process`-проводка,
  ...
  строчкой в stderr (*держится для runtime-канала — HATS-1862: объявленный
  хук, чей скрипт не там, где должен быть, никогда не выпадает из манифеста
  молча; нет файла в самом скилле — warn в стартовых notices и строки нет,
  файл в скилле есть, а в зеркале сессии нет — `RuntimeHookMirrorError`,
  сборка отказывает; HATS-1439 назвала дыру и закрыта как superseded*).
  *Сторож: сам гейт резолва — отказ громкий по построению; на
  detached-половину —
  `tests/e2e/test_githooks_orchestrator.py::test_a_commit_lands_when_ai_hats_is_gone`;
  на runtime-половину — `tests/test_composed_rows.py` (обе ветки таксономии и
  паритет dry-run), `tests/e2e/test_runtime_hook_script_missing_warns.py`
  (warn доходит до баннера, сессия стартует),
  `tests/test_runtime_hook_mirror_missing_refuses.py` (отказ до запуска).*
...
- **M13 — Наблюдаемость.** Материализация, свипы и heal отчитываются о
  ...
  а не только строчкой в stderr. *Для runtime-хуков держится (HATS-1862):
  пропавший скрипт идёт в `artifacts.notices` — стартовый баннер HITL и
  sys-трейс сабагента. Для свипов и heal — цель; сторожа нет.*
...
| ~~HATS-1439~~ | ~~сессия сужает общие hook-директории под другими живыми сессиями~~ — **закрыт** как superseded: общих директорий нет (HATS-1268/1269/1337/1480), сигнал о выключенном гейте — HATS-1862                                                        | M2, M7, M10, M13 |
| ~~HATS-1862~~ | ~~писатели манифеста runtime-хуков (claude, codex, cline, opencode) молча выбрасывают хук с неразрешившимся скриптом; agy не проверяет вовсе~~ — **закрыт**: один `composed_rows` (`hook_collection.py`) на все пять, warn/refuse по виновнику  | M10, M13         |
...
набор; канал прописывает набор в своей точке записи (M15). Строки
runtime-манифеста у всех пяти поверхностей даёт один проход —
`composed_rows` (`hook_collection.py`): скрипта нет в самом скилле — warn в
`artifacts.notices` и строки нет; скрипт в скилле есть, а в зеркале сессии
нет — `RuntimeHookMirrorError`, сборка отказывает. Есть ли файл в зеркале,
отвечает порт (`Materializer.executable_at`), так что dry-run отвечает так
же, как реальная сборка (HATS-1862):
