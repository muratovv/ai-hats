...
- **checks — *реализовано* на рёбрах `edge:` (HATS-1141) и в точке
  ...
  пишет: на `edge:` — `<tasks_dir>/<ID>/.checks/<точка>~<токен>.log`,
  то есть под тем tasks-каталогом, ПРОТИВ которого идёт переход (при
  `--tasks-dir` вне проекта — вне проекта); на `pre-merge` —
  ...
  *реализовано (HATS-1540)* — своего снапшота у канала больше нет),
  ...
  шелл-аут в нём запрещён AST-пином. Материализация от этого не изменилась ни в
  одном корне: перевозится тот же уже разрешённый ряд, точки записи как не было,
  так и нет.
