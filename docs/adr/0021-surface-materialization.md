...
| ~~HATS-1480~~ | ~~плоская копия `packages/ai-hats-library/src/ai_hats_library/hooks/` + манифест + drift-арм ещё живы~~ — **закрыт**: копия `packages/ai-hats-library/src/ai_hats_library/hooks/` + `.manifest` снесена, drift-net ретайрен, residue выметен миграцией step 10                                                            | M5, M8           |
...
| **проект, `<ai_hats_dir>`** | каноническая композиция + `user-rules/`, `library/{rules,skills}`, трекер, `sessions/runs/`                                                                                                                                                                                                                                                                                                                                    | install-time `_refresh` и `set_role` (`write_canonical`); rack (трекер); observe (`runs/`)                                                                              | проект                                                                                                     | `packages/ai-hats-library/src/ai_hats_library/hooks/` + `.manifest` — снесён (HATS-1480, done — миграция step 10; проводка отцеплена от неё в HATS-1268); `library/wt-hooks/` + `.manifest` — снесён (HATS-1269, done — миграция step 9); `tracker/lifecycle-hooks/` — снесён (HATS-1147, done); `sessions/runs/` ограничен двухслойным удержанием (HATS-1339, done)                                                                                                                                    |
...
директории с манифестами ретайрнуты (`packages/ai-hats-library/src/ai_hats_library/hooks/` — HATS-1480,
...
1. **Общие hook-директории** (баг-карточка **HATS-1439**) — **растворён**
   ...
   `packages/ai-hats-library/src/ai_hats_library/hooks/`, `library/wt-hooks/` и `.githooks/` из композиции
...
`packages/ai-hats-library/src/ai_hats_library/hooks/`) — иллюстрации, не контракт: у другого проекта состав
