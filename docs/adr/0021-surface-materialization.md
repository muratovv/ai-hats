...
ADR владеет картиной материализации целиком**: подготовка роли на каждой
поверхности, интеграция хуков в неё, install-time установка, параллельные
сессии, кэш и очистка. Карта чокпойнтов/ярусов и требования Mx, добавленные
...
готовится роль на каждой поверхности; как в неё интегрируются хуки; что
...
### S1 — Ярусы: кто какой байт пишет и сколько он живёт

| ярус                         | пути                                                                                                                                                                                                                        | writer / чокпойнт                                                                                                      | лайфтайм                                                         | to-be дельта (владелец)                                                                                                                                                                                                                                                                                                                                                                                  |
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| **глобальный ярус, `$HOME`** | `~/.gemini/antigravity-cli/settings.json` (запись agy-диспетчера); `<cache_home>/<project-key>/` (дефолт `~/.cache/ai-hats/`)                                                                                               | сборка agy-сессии — **каждая**, не init (`agy/provider.py:206`, вопреки ADR-0018 §5.1); сборка сессии + recovery-свипы | пользователь / машина                                            | read-modify-write получает лок или CAS (HATS-1338); каждый класс кэша ограничен liveness-aware GC (HATS-1339)                                                                                                                                                                                                                                                                                            |
...
### S2 — Подготовка роли на каждой поверхности
...
`<sc>`) — вне воркспейса (HATS-1398, S4).
...
### S3 — Интеграция хуков в поверхность
...
### S4 — Что ставится на `self init` / `self update`
...
init **не** регистрируется (только на сборке сессии — S3). Re-init
...
шага (M11); bump-гейт резолва остаётся.

### S5 — Параллельные сессии
...
### S6 — Кэш: структура и использование
...
- `sessions/<sid>/` — дерево сессии (S2): его читают лончер-флаги
...
- TTL-свип **слеп к живости**: сессии дольше 24 ч реальны (наблюдался agy-PID
  с аптаймом 25:50) — свип может снести кэш живой сессии. Цель M8:
...
### S8 — Требования к материализации
...
- **M4 — Чистый корень.** Ни одна запись провайдера или сессии не попадает в
  ...
  (HATS-1338).*
- **M5 — Неизменность сессии.** Сессия исполняет одни и те же байты от
  ...
  (HATS-1269).*
- **M6 — Конкурентная безопасность.** Конкурентные сессии с разными ролями
  ...
  (HATS-1338).*
- **M7 — Нет второй копии — нет полиции.** Ни один скилловый скрипт не
  существует во второй, отдельно управляемой копии; то, что не может
  дрейфовать, не нуждается ни в манифесте, ни в drift-арме, ни в детекторе
  утечек. Карточка, удаляющая копию, в себе же удаляет её сторожей. *Цель
  (HATS-1268/1269/1337; четверть `lifecycle_hooks` — done, HATS-1147).*
- **M8 — Ограниченный диск.** У каждого класса материализованных артефактов
  ...
  вовсе; замеры в S7).*
- **M9 — Поведение при отказе следует таксономии.** `in_process`-проводка,
...
- **M10 — Containment при резолве.** Каждый путь, разрешённый из
  ...
  HATS-1337, HATS-1443).*
- **M11 — Переживает `self update`.** Долговечные артефакты не содержат
  путей в версионированный venv; после update следующий коммит/сессия
  запускает гейты *новой* версии без шага перематериализации. *Цель
  (HATS-1337).*
- **M12 — Наблюдаемость.** Материализация, свипы и heal отчитываются о
...
меняющая чокпойнт, ярус или политику очистки, правит S-секцию здесь — иначе
