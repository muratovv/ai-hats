...
- **M2 — Проект меняется только на init/update.** Всё, что переживает
  ...
  живые сессии. *Цель (HATS-1268/1337; wt-половина — done, HATS-1269; баг — HATS-1439). Сторож:
...
- **M5 — Ролевая нейтральность стейта проекта.** Запуск роли не переписывает
  ...
  наблюдаемости). *Цель (HATS-1268/1337/1338; wt-половина — done, HATS-1269). Сторож:
...
- **M6 — Неизменность сессии.** Сессия исполняет одни и те же байты от
  ...
  что она запускает. *Держится для промпта/скиллов/agy-хуков и для wt-хуков
  (HATS-1269, done); цель для hook-скриптов claude (HATS-1268). Сторож:
  `tests/e2e/test_session_wiring_survives_later_materialization.py`
  (RED-xfail, HATS-1439).*
...
- **M8 — Нет второй managed-копии hook-скрипта.** Речь именно о
  ...
  удаляет её сторожей. *Цель (HATS-1268/1337; `wt-hooks` — done, HATS-1269;
  `lifecycle_hooks` —
...
- **M11 — Containment при резолве.** Каждый путь, разрешённый из
  ...
  git — цель (HATS-1337, HATS-1443). Сторож wt-половины:
...
| **проект, `<ai_hats_dir>`** | каноническая композиция + `user-rules/`, `library/{rules,skills}`, трекер, `sessions/runs/`                                                                                                                                 | install-time `_refresh` и `set_role` (`write_canonical`); rack (трекер); observe (`runs/`)                             | проект                                                           | `library/hooks/` + `.manifest` — **исчезает** (HATS-1480; проводка отцеплена от неё в HATS-1268); `library/wt-hooks/` + `.manifest` — снесён (HATS-1269, done — миграция step 9); `tracker/lifecycle-hooks/` — снесён (HATS-1147, done); `sessions/runs/` ограничен GC (HATS-1339)                                                                                                                                                                   |
...
| **сборка сессии**           | каждая сессия (HITL и automate)                | recovery-свипы эфемерного состояния (TTL кэшей-сирот, недособранные версии — `environment_recovery.py:122-166`); артефакты по поверхностям в дерево сессии. Сегодня (до HATS-1268/1337) сюда же вклинивается drift-heal `sync_hooks` (`wrap_runner.py:558`), перематериализующий **общие** hook-директории из композиции этой сессии — вектор HATS-1439                                                                                   |
...
4. **Сборка артефактов** билдером по категориям (эта секция): промпт →
   ...
   (S8). *Сегодня (до HATS-1268/1337) сюда же вклинивается drift-heal
...
**Сегодня (до HATS-1268/1337)** `hooks.materialize` ставит три
