...
- **M2 — Проект меняется только на init/update.** Всё, что переживает
  ...
  живые сессии. *Цель (HATS-1268/1269/1337; баг — HATS-1439). Сторожа нет —
  класс 1439 сегодня ловится только ручным прогоном chain-харнесса
  (`tests/e2e/_helpers/hook_chain.py`); настоящий сторож появится с
  растворением копий.*
...
- **M4 — В корне проекта ai-hats владеет двумя вещами.** `.githooks/` и
  ...
  нарушитель — цель (HATS-1338). Сторож: частичный —
  `test_root_carries_no_ai_hats_hook_commands_after_sweep` (только
  hook-проводка) и goldens поверхностей (`escapes: []`); единого сторожа
  «в корне не появилось вообще ничего» нет.*
- **M5 — Ролевая нейтральность стейта проекта.** Запуск роли не переписывает
  ...
  наблюдаемости). *Цель (HATS-1268/1269/1337/1338). Сторожа нет (частично
  закрывают goldens поверхностей).*
- **M6 — Неизменность сессии.** Сессия исполняет одни и те же байты от
  ...
  hook-скриптов claude (HATS-1268) и wt (HATS-1269). Прямого сторожа нет.*
...
- **M8 — Нет второй managed-копии hook-скрипта.** Речь именно о
  ...
  done, HATS-1147). Сторож: `tests/e2e/test_lifecycle_hooks_retired.py`
  (тумбстоун снесённого канала); на живые копии появится со сносящими
  карточками.*
- **M9 — Ограниченный диск.** У каждого класса материализованных артефактов
  ...
  HATS-1473). Сторож: `tests/e2e/test_session_cache_out_of_tree.py` (кэш
  вне воркспейса); на политики GC — сторожа нет.*
...
4. **Сборка артефактов** билдером по категориям (эта секция): промпт →
   ...
   (S8). *Сегодня (до HATS-1268/1269/1337) сюда же вклинивается drift-heal
   общих hook-директорий (`wrap_runner.py:558`) — вектор HATS-1439, S7.*
