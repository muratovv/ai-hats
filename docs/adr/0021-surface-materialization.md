...
- **`[rules]`** — резолвленные правила `(имя, source_path)`. В тело промпта
  ...
  `constants.py`). Не-always-on правило до агента доводит инжекция его
  трейта: выжимка + указатель «see rule X»; pre-commit гейт rule-delivery
...
- **`[checks]`** — будущий канал (ADR-0019 [2]): проверки на рёбрах FSM
  бэклога и wt-точках («нельзя `review→done` с неразобранными
  ревью-заметками»). Уходят в снапшот `checks/` сессии.
...
| `[checks]`        | снапшот `checks/` сессии (будущее)                     |
...
- **M2 — Проект меняется только на init/update.** Всё, что переживает
  ...
  живые сессии. *Держится для runtime-хуков claude (HATS-1268); цель для wt
  (HATS-1269) и git (HATS-1337). Сторож:
  `tests/e2e/test_session_wiring_survives_later_materialization.py` —
  GREEN-пин с HATS-1268; он же был единственным воспроизведением HATS-1439,
  чья runtime-половина этим и растворена.*
...
- **M5 — Ролевая нейтральность стейта проекта.** Запуск роли не переписывает
  ...
  наблюдаемости). *Цель (HATS-1269/1337/1338; для runtime-хуков claude снято
  HATS-1268 — проводка больше не смотрит в общую директорию). Сторож:
  `tests/e2e/test_role_switch_does_not_narrow_hooks.py` (RED-xfail, HATS-1480 —
  дефект снимается удалением копии, и та же карточка переписывает тест: после
  удаления его предусловие неудовлетворимо).*
- **M6 — Неизменность сессии.** Сессия исполняет одни и те же байты от
  ...
  что она запускает. *Держится для промпта/скиллов/agy-хуков и, с HATS-1268,
  для hook-скриптов claude; цель для wt (HATS-1269). Сторожа:
  `tests/e2e/test_session_wiring_survives_later_materialization.py` (GREEN-пин)
  и `tests/e2e/test_runtime_hooks_execute_from_session_tree.py` (команда
  резолвится внутрь дерева сессии, сосед-файл данных на месте, хук ПИШЕТ в
  журнал обходов — заглушка возвращает ноль и по коду неотличима).*
...
- **M8 — Нет второй managed-копии hook-скрипта.** Речь именно о
  ...
  удаляет её сторожей. *Цель (HATS-1480/1269/1337; `lifecycle_hooks` — done,
  HATS-1147; проводка claude отцеплена от копии в HATS-1268, сама копия ещё
  жива). Сторож: `tests/e2e/test_lifecycle_hooks_retired.py` (тумбстоун
  снесённого канала); на живые копии появится со сносящими карточками.*
...
- **M10 — Поведение при отказе следует таксономии.** `in_process`-проводка,
  ...
  stderr (*цель — HATS-1439 называет дыру*). *Сторож: сам гейт резолва —
  отказ громкий по построению; на detached-половину появится с HATS-1337.*
- **M11 — Containment при резолве.** Каждый путь, разрешённый из
  ...
  Отдельного сторожа нет — тест обязателен в этих карточках.*
- **M12 — Git-гейты переживают `self update` без переустановки.**
  ...
  такого требования нет — оно перестраивается на каждом старте (M6). *Цель
  (HATS-1337). Сторожа нет; e2e «commit после update без перематериализации»
  — критерий приёмки HATS-1337.*
...
| HATS-1480     | плоская копия `library/hooks/` + манифест + drift-арм ещё живы; шесть in-process сторожей ждут репойнта    | M5, M8           |
...
> Из set_role хуковая материализация уходит с HATS-1268/1269/1337,
...
| **проект, `<ai_hats_dir>`** | каноническая композиция + `user-rules/`, `library/{rules,skills}`, трекер, `sessions/runs/`                                                                                                                                 | install-time `_refresh` и `set_role` (`write_canonical`); rack (трекер); observe (`runs/`)                             | проект                                                           | `library/hooks/` + `.manifest` — **исчезает** (HATS-1480; проводка отцеплена от неё в HATS-1268); `library/wt-hooks/` + `.manifest` — **исчезает** (HATS-1269); `tracker/lifecycle-hooks/` — снесён (HATS-1147, done); `sessions/runs/` ограничен GC (HATS-1339)                                                                                                                                         |
...
| **сборка сессии**           | каждая сессия (HITL и automate)                | recovery-свипы эфемерного состояния (TTL кэшей-сирот, недособранные версии — `environment_recovery.py:122-166`); артефакты по поверхностям в дерево сессии. Сегодня (до HATS-1480/1269/1337) сюда же вклинивается drift-heal `sync_hooks` (`wrap_runner.py:558`), перематериализующий **общие** hook-директории из композиции этой сессии; для runtime-хуков claude это перестало быть опасным с HATS-1268 — их проводка туда не смотрит  |
...
4. **Сборка артефактов** билдером по категориям (эта секция): промпт →
   ...
   (S8). *Сегодня (до HATS-1480/1269/1337) сюда же вклинивается drift-heal
   общих hook-директорий (`wrap_runner.py:558`) — S7.*
...
6. **Работа**: spawn хуков — S5. Runtime-хуки порождает сама поверхность,
   wt-хуки — процесс ai-hats, git-хуки — git через `.githooks/`.
...
- **Старт без обслуживания.** Automate не выполняет ничего из
  HITL-стартового набора: ни drift-heal `sync_hooks`, ни линтов
...
- **runtime, claude — целевая форма с HATS-1268:** проводка сессии
  (`settings.json`) указывает абсолютным путём в зеркало скиллов
  `<sid>/plugin/skills/<skill>/hooks/<file>` — те же байты, что и у
  agy-модели; соседние файлы данных скилла на месте по конструкции. Каждая
  запись **скилловая**, включая shared-state-гард: он переехал в core-скилл
  `safety-guard` вместе с `shared_state_classifier.sh`, хардкод-запись
  `ai-hats:hats-437` снята. Привязка стала композиционно-гейтед, поэтому
  `safety-guard` висит на `trait-base` и `trait-analyst-base` — замер до/после
  дал 19 ролей из 19 в обоих случаях. Разделяемый `bypass_journal` лежит рядом
  с каждым потребителем относительной ссылкой на канон (оговорка к
  `bundle: dir`, ADR-0020 [3] D1). *Сегодня (до HATS-1480):* директория `library/hooks/` ещё пишется и ещё
  метётся — на неё просто больше никто не смотрит; её ретайр вместе с
  drift-армом и репойнтом шести in-process сторожей — HATS-1480. Про
  double-fire: корень вычищен заранее (HATS-1336, done), потому что дедуп
  claude 2.1.220 держался на побайтовой идентичности написаний (замер
  2026-08-01), а перенацеливание её разрушает — порядок рёбер был обязателен,
  не желателен.
...
**Сегодня (до HATS-1480/1269/1337)** `hooks.materialize` ставит три
managed-поверхности; замер на этом репозитории:

1. `<ai_hats_dir>/library/hooks/` + `.manifest`: пакетные хелперы (с
   HATS-1268 это `bypass_journal.{py,sh}` — гарды уехали в скиллы) ∪
   по-скилловые флэттен-копии `<skill>-<basename>`, mode `0o755`
   (`hooks_manager.py:222-261`). Число скилловых копий зависит от
   композированной роли, поэтому здесь не приводится. **Проводка claude в
   эту директорию больше не смотрит** (HATS-1268) — она остаётся до
   HATS-1480 как ещё-пишущаяся, но никем не читаемая.
...
1. **Общие hook-директории** (баг-карточка **HATS-1439**; фикс —
   растворение копий, HATS-1480/1269/1337). Session-start drift-heal
