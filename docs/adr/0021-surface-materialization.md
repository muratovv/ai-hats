...
- **Владелец сессионного кэша объявлен.** На старте сессии в её дерево
  ложится якорь `.session-owner.json` — `{root_pid, start_time}`
  (`write_session_anchor`, `session_liveness.py`); точка записи —
  `_claim_session_cache` (`runtime_common.py`), её зовут оба раннера.
  `start_time` из `ps` делает запись устойчивой к переиспользованию pid.
  ...
  возрастном TTL (`SESSION_CACHE_TTL_HOURS = 24`), и только такой каталог.
...
- **`runs/` — два слоя удержания** (`runs_retention.py`). Слой фактов —
  ...
  возрасту (`BULK_MAX_AGE_DAYS = 14`). Единица истечения — **файл**, не
  каталог: каталог прогона не удаляется никогда, поэтому id сессии всегда
  резолвится. Набор истекаемых — конечный аллоулист (форма `retired_dists`,
