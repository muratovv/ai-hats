...
- **`[checks]`** — проверки на рёбрах FSM бэклога и wt-точках («нельзя
  ...
  *реализовано (HATS-1540)*; остальные объявленные точки вызывающего не имеют
  (ADR-0019 [2] D3, владельцы HATS-1577 и HATS-1578). **Единого каталога точек
  нет: с HATS-1541 и HATS-1545 имена валидирует то приложение, которое точку и
  зажигает.** ai-hats знает имена одного приложения — своего, `wt`
  (`check_points.wt_points()`); груз всех прочих, `apps.rack` включительно, он
