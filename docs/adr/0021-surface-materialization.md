...
- **`[checks]`** — проверки на рёбрах FSM бэклога и wt-точках («нельзя
  ...
  *реализовано (HATS-1540)*, собственного снапшота `checks/` у канала больше нет.
  ...
  (ADR-0019 [2] D3). **С HATS-1541 каталог покрывает только СВОИ пространства
  имён** (`card:`, `wt:`): имя точки чужого приложения материализация перевозит
  непрозрачно, а разбирает, фильтрует по своей топологии и подписывается на
  рёбра сам рак (ADR-0019 [2] D11). Материализации это не добавляет ни одной
  точки записи — набор по-прежнему заморожен в композиции.
