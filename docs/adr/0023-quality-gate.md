...
— в HATS-1614, до конца — в HATS-1664 (предмет ребра `->done` и дорога, на
которой его можно прогнать). D2 закрыт следствием того же: второй точки не
потребовалось.
...
**Замеры сняты 2026-08-14** (HATS-1655), кроме отдельно помеченных. Счётчики
...
| **push**      | `pre-push` в master  | человек           | пушимый sha               | полный `e2e`-тир + `adr-integrity` (HATS-1646)                                                          |    ~25–27 мин |
...
- **Полный e2e-тир на каждой карточке.** 25–27 минут против четырёх; тир остаётся
  на push-time.
...
Первые четыре **выведены**, последние два **наблюдались**:
...
1. **Снятие оверлеем** — если оверлей убирает скилл, строка выбывает, и весь
   ...
   строку как ARMED / FOREIGN / DEAD (`classify_bindings` в `checks.py`,
...
между именами). HATS-1603 (бюджет чека против лока) решена отдельно: бюджет 20 с
