...
одной операцией сравнения**. Поглощение при этом сегодня не срабатывает ни разу
— составы двух живых гейтов не вложены (`done-gate` = {e2e-catalog, lint, unit,
integration, merge-smoke}, `push-gate` = {lint, unit, e2e-catalog, e2e});
первый потребитель — **HATS-1614** (`->merge ⊂ ->done`).
...
диспетчера на `<gate> --stages`. Проект, который на этот вопрос не отвечает,
