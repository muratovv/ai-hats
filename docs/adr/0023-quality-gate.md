...
> 4545 тестов из 5625, `integration` — 400 из 4915, `coverage` — 4915. Причина —
> **30 тестов из 8 файлов `tests/e2e/`, не помеченных `integration`** вопреки
> `dev_rule_e2e_gate`: они едут в `unit` и невидимы `integration`. Пока это не
> починено, «unit + integration» не значит написанного. Владелец — **HATS-1600**.

> **Сегодня.** `ruff format --check` покрывает `src/ tests/` (`ci-local.sh:50`),
> и вердикт тира линтинга зависит от версии инструмента. Один замер 2026-08-12 на
> одном дереве: venv основного чекаута (ruff 0.15.22) — «956 files already
> formatted», rc=0; свежий venv воркtree (ruff 0.16.2) — «5 files would be
> reformatted, 1299 already formatted», rc=1, потому что 0.16.2 форматирует
> code-блоки в markdown. Пин `ruff>=0.4` не имеет потолка, поэтому ответ гейта
> определяется датой создания venv: **HATS-1607**, и он идёт раньше расширения
> области (**HATS-1408**, **HATS-1415**), потому что меняет её цену.
...
> перечисленное написано руками дважды, в `git_hooks/pre-push-e2e-master.sh` и
> `hooks/done-gate.sh`, включая расходящийся контракт кодов возврата. Владелец —
> **HATS-1604**.
...
> integration -> merge-smoke». Текст отказа обязан рендериться из состава.
...
Наследуются любой новой строкой:
