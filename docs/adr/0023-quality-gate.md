...
| `ticket-ids`       | review-gate done-gate merge-gate push-gate                                               | no tracker id in library prose, a doc a reader opens, or src/                                                           |
...
HATS-1991 её не требует ни один гейт). Под xdist (`-n8`, 14 ядер),
2026-09-16 (HATS-1673): `merge-smoke` 6,6 с, `unit` 62 с; `review-gate`
целиком с нуля вместе с зонами диффа (`gates`, `rack`) — 183 с.
...
`review-gate` штампует всё, что требует сам; `make done-gate` после него гоняет
только разницу составов (таблица выше). Дерево слияния
