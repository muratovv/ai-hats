...
`provision-venv.sh`, что получает каждый task-воркtree). Без него `$PY`
свалился бы на внешний интерпретатор, и реально-подпроцессные стадии
исполняли бы код ГЛАВНОГО чекаута, объявляя это судом над коммитом.
