...
2. **Запись.** Каждый обход пишется через `ai_hats_journal_bypass()`
   (`hooks/bypass_journal.sh`) [7]. Адрес зависит от того, знает ли запись свою
   сессию: знает — `<runs>/session_<id>/bypasses.jsonl`, рядом с `audit.md`; не
   знает — общий `<git-dir>/ai-hats/bypasses.jsonl`. Так всё про одну сессию
   лежит в одном каталоге, а git-тир, у которого сессии часто нет, сохраняет
   прежний адрес (HATS-1634).
3. **Всплытие.** На следующем пуше `pre-push-bypass-report.sh` печатает
   человеку, сколько обходов проехало в этих коммитах — читая **оба** дома.

Симметричное свойство — **сработка** тоже пишется, иначе «гейт не поймал» и
«гейта не было» неразличимы (HATS-1634). Отказывая, эскалируя или предупреждая,
гейт зовёт `ai_hats_journal_catch()` / `journal_catch()`. Две точки записи
покрывают по несколько гейтов каждая: `safety_gate._emit`, куда сходятся все
четыре его не-allow ветки, и `lib/gate.sh::gate_exit`, которую сорсят три
quality-гейта. Литеральные `exit 1` выше `gate_exit` намеренно **не** прошиты:
каждый означает, что гейт сломался, а не отказал.

Запись ложится в `<runs>/session_<id>/catches.jsonl`. Файл отдельный от
`bypasses.jsonl` по двум причинам: `pre-push-bypass-report.sh` грепает по SHA без
фильтра по `kind`, так что catch там читался бы как обход; и схема полей своя
(`CATCH_FIELDS`) — без `head_before`/`branch`/`sha`, потому что это поля аудита,
`stamp_sha` к catch не применяется, а на hot-path они стоят двух лишних
git-вызовов. Замерено: catch 8.2 мс на запись против 25.6 мс у bypass; на
allow-ветке не вызывается ничего.

Атрибуция берётся из пути самого хука (`.../sessions/<id>/plugin/...`), а не из
окружения: харнесс поднимает runtime-хуки без `AI_HATS_SESSION_ID` — 5 записей
из 3898. `AI_HATS_DIR` в резолюции не участвует намеренно: унаследованный из
другого чекаута, он назвал бы чужой трекер. Покрытие держит храповик
`tests/test_catch_journal_coverage.py`, выводящий список гейтов из библиотеки и
засчитывающий сорснутое тело как своё, — поэтому новый хук не может тихо не
писать.
...
на этих тестах плюс префикс пути, который ими владеет. Одна строка `gates.sh
zones` (`packages/ai-hats-rack/ | rack | e2e-rack`). Каждая зона — своя
именованная стадия, а `e2e-default` — половина, не занятая ни одной зоной.

- **Карточный гейт требует объявленное множество плюс зоны, которых требует
  ...
  команда, которую он выдаёт, не заработала бы названную стадию.
...
пересечение пусто — `tests/test_e2e_zone_partition.py` спрашивает сами стадии
через `--collect-only`. Копия их marker-выражений в тесте была бы тем самым
