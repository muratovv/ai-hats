...
— в HATS-1614; остаток D4 держит HATS-1615.
...
> `CHECK_PRIORITY = 15` (`checks.py:29`), слияние делает воркtree-подписчик на 30.
>
...
Состав при этом читается из диспетчера **судимого дерева** (D7): пока жив
воркtree — из него, после слияния — из главного чекаута, потому что судимое
дерево теперь там.
...
> (`WorktreeDriftError`, `manager.py:2161`), а содержащая ветка сливается в своё
