...
стал **строкой таблицы, а не файлом**, маркер — **постадийным**, механизм —
**трёхслойным** (D6). Прежняя история исполнения — HATS-1604 (примитив), HATS-1614
...
hook-подложкой и контрактом кодов возврата — в том числе грузом строки, который
с HATS-1878 доезжает до хука. **ADR-0021** [3] владеет материализацией
...
генерируются из `scripts/gates.sh` и привязок роли (`scripts/gen_gate_table.py`
[5]); стадия `gate-table` отказывает документу, отставшему от кода. Всё вокруг них
...
- **Гейт** — **имя для множества стадий**, объявленное в `scripts/gates.sh`.
  Единственное место в коде, где есть имена гейтов. Таблица там — строки-стадии:
  `стадия | гейты, которые её требуют | что она проверяет`, в порядке прогона.
  Стрелки в таблице **нет**: где гейт применяется — вопрос роли.
- **Примитив гейта (`ci-gate`)** — `scripts/ci-gate.sh`: принимает СПИСОК стадий
  и не знает имён гейтов. `check` — требование (какие стадии не отмечены для
  дерева-субъекта; ничего не запускает), `run` — исполнение (гоняет неотмеченные,
  штампует каждую зелёную).
...
**Закрыто** (D6): состав объявляется один раз, в таблице `scripts/gates.sh`.
...
его **правилом без флага**. `ci-gate.sh run` берёт субъект (`HEAD` или
...
**Требование читается из таблицы судимого дерева.** На проверке хук зовёт
`<дерево>/scripts/gates.sh check <gate>` — из воркtree карточки, пока она жива,
из главного чекаута с `--rev <мёрдж-sha>`, когда её нет. Карточка, меняющая
гейт, судится по правилу, которое сама везёт (`test_the_composition_is_read_from_the_tree_under_judgement`).
...
`ci-gate.sh check` не имеет пути к раннеру стадий — не «не должен», а не может;
...
**Раннер инъектируется** (`CI_GATE_STAGE_RUNNER`): без этого каждый тест L2 стоил
бы настоящего `unit`; с ним вся логика «пропустить/прогнать/штамповать/
остановиться» тестируется на фейковом раннере за миллисекунды, а настоящий — под
e2e.

**Адаптер checks-канала — один файл на все рёбра.** `hooks/gate.sh` узнаёт,
какой он гейт, из груза строки, которая его породила: `gate: done-gate` рядом с
`run:` доезжает как `AI_HATS_CARGO_GATE` (ADR-0020 [2] D2, правка HATS-1878).
Строка без `gate:` — отказ, не пас: хук, не знающий, кто он, не может ничего
проверить. Адаптер git-канала — отдельный файл нарочно: его дефолтный режим
читает протокол pre-push из stdin, а раннер checks отдаёт детям `stdin=DEVNULL`
— пустой stdin попал бы в fast path «нет master-цели» и гейт был бы вечнозелёным.

### D7 — Граница: библиотека не называет даже имени гейта

Движок владеет **точками, подложкой хука и контрактом кодов возврата**. Проект
владеет **стадиями, таблицей гейтов и маркерами** (L1–L3). Роль владеет **тем,
какой гейт стоит на каком ребре** — строкой `composition.apps` с `gate:`.
Библиотечный файл не утверждает ничего проектного: до HATS-1878 имя гейта было
именем файла в библиотеке (`done-gate.sh`) — ровно то утверждение, которое D7
запрещал, закодированное в filename. Проект, у которого нет `scripts/gates.sh`
или чья таблица не знает названного гейта, получает отказ, а не откат на состав,
который библиотека назвала бы сама.
...
- **Генерировать файл гейта из таблицы** (HATS-1878). Ноль правок движка — но
  файлы остались бы, а поставленная форма — «добавить строку, не файл».
- **Выбирать гейт по `AI_HATS_HOOK_POINT`** (HATS-1878). Ноль правок движка, но
  «где применяется» жило бы в двух домах (таблица и `at:`), и матчить пришлось бы
  `selector` внутри JSON-конверта. Оставлен запасным ходом, если груз строки
  когда-нибудь потребует интерпретации движком.
...
четвёртая копия — отставал на семь стадий. Закрыто таблицей (L3), одним хуком,
выбираемым грузом строки (D6), и `gate-table`, который отказывает отставшему
документу.
...
- [2] ADR-0020 — hook-подложка, контракт кодов возврата и груз строки:
