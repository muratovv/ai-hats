...
**`->done` спрашивает: зелен ли master после этой карточки.** Ребро судит
**результат слияния**: пока воркtree жив — дерево ветки, после слияния — дерево
мёрдж-коммита, чей sha приезжает в окружении чека (HATS-1664).
...
## Пять способов гейту исчезнуть тихо

Первые три наследуются любой новой строкой; четвёртый не про строку, а про
дорогу; пятый — про роль, которая строку не несла:
