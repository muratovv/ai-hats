...
- **C2** — `hook_exec.run_hook` объявлен в [3] единым примитивом, а владеет
  четырьмя каналами из восьми. Один из четырёх обходов молча превращает отказ в
  успех: цепочка git-хуков ловит `OSError` и продолжает, а при провале композиции
  пропускает все гейты — и в обоих случаях отдаёт git код 0.
...
- **C5 — единственная без кандидата в владельцы.** Признак его отсутствия
  сегодня: бюджеты ярусов объявлены порознь и несовместимо — 45 с в
  `worktree_hooks`, 60 с у codex-диспетчера без env, 60 с у agy-диспетчера со
  своей env, а цепочка git-хуков не объявляет бюджета вовсе. Прежний признак —
  `assert` соотношения бюджетов во время импорта — снят в пользу вычисляемого
  `Deadline.budget_for`, но хозяина бюджетам это не создало. Владельца придётся
  создавать.
...
| **C5**  | владелец бюджетов ярусов — **не назначен**                                             | `core.file_lock`, замки wt, замок ядра rack                                          | `ai_hats/locks/` или `core` — **решить**            | объявление ярусов и бюджетов                                                                                 | `core`                              | **stateless**; файлы замков — не схема                                                                         | бюджета яруса, объявленного вне владельца, нет: каждый выводится через `Deadline.budget_for` от объявленного яруса |
...
| **C14** | модуль `install`                                                                       | лаунчер, `_bootstrap.py`, диспетчер — не могут разделить код                         | `ai_hats/install/` + `_bootstrap.py` на stdlib      | API починки и обеспечения                                                                                    | `core`; у `_bootstrap` — **ничего** | владеет схемой venv-метки и её миграциями                                                                      | три бутстрапа дают один venv-контракт — конформанс, проверяемый e2e на реальном venv (`dev_rule_e2e_gate`)         |
...
**Форма гейта: только отрицательный универсал.** Проверяющий тест закрывает
строку, лишь когда утверждает **отсутствие второго пути** — «вне X нет», «ровно
один», «второго диспетчера нет». Тест, утверждающий свойство поведения, верен и
при нескольких владельцах, а тест, описывающий собственную форму («проверяется
тестом, а не ассертом»; «e2e на реальном venv»), не утверждает о коде ничего.
Цена этой разницы уже заплачена: прежний гейт C5 требовал, чтобы соотношение
бюджетов держал тест, а не `assert` на импорте, — `assert` убрали, тест написали,
гейт позеленел, и бюджеты остались бесхозными в четырёх местах. Гейт, который
можно закрыть, не тронув вторую реализацию, метрику D1 не наполняет, а разбавляет.
Колонка приведена к этой форме **пока только у C5 и C14**; построчная ревизия
остальных — [14].
...
и следствия перепроверены по коду 2026-08-21:

| # | Способность                              | Живое следствие                                                                                                          |
| - | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| 1 | Разрешение проекта и конфига (`Project`) | два резолвера расходятся и по маркерам, и по политике чужого пина, и по отказу: rack бросает, CLI молча возвращает `cwd` |
| 2 | Исполнение хуков                         | 4 канала из 8 идут мимо владельца, и один из них — цепочка git-хуков — отдаёт код 0 на несработавшем гейте               |
| 3 | Материализация                           | `--dry-run` отчитывается о четырёх корнях, в которые не смотрел                                                          |
| 4 | Композиция роли                          | политик отказа четыре, и две из них сходятся в одной команде `rack transition`                                           |
| 5 | Ownership / блокировки                   | четыре несовместимых бюджета ярусов, из них у цепочки git-хуков бюджета нет вовсе                                        |
| 6 | Идентичность сессии                      | литерал переменной объявлен в 8 файлах пяти дистрибуций плюс дважды в шелл-шаблонах (свёртка HATS-1826 свела знаменатель к четырём — числитель не перемерян) |
| 7 | Диспетчеризация пайплайнов               | второй диспетчер мимо `PipelineHarness`: без его namespace, GC, проводки трейсов и `load_user_steps`                     |

Слайсы 2 и 3 — **доведение уже ратифицированного** ([3] и [4]), а не новое
проектирование.

**Что изменила перепроверка 2026-08-21.** Строка 5 прежде ссылалась на `assert`
соотношения бюджетов во время импорта — он удалён, инвариант считает
`Deadline.budget_for`; живое следствие теперь в разнобое самих бюджетов. Строка 6
прежде называла четырёх приватных читателей переменной — они ушли за
`SessionIdentity`, а болезнь переехала в переобъявление литерала по дистрибуциям.
Строка 7 сужена до C9: C7 из неё выбыл (записи мимо ядра нет — это же утверждает
и целевая таблица), а C8 заменён решением по [8] и проверяемого дефекта записи
больше не формулирует.
...
**Целевое состояние.** Модульный отбор тестов включается **не для базы, а для
области**, и только когда про неё выполнено всё:

1. D5 действует и **enforcing** для этой области;
...
Область, для которой пункты 1–3 не выполнены, гоняет свои тесты всегда. Это
нулевое значение по умолчанию, а не регресс: T2 наступает по областям и ровно
настолько, насколько каждая из них доказала свою границу.

**Почему не сразу и почему в таком порядке.** Селектор читает граф, а граф этой
базы на 45% состоит из отложенных рёбер, и все циклы проходят только через них:
построенный на модульном графе, он увидит чистый DAG из 316 рёбер там, где рёбер
576, — будет слеп к 240 связям и станет пропускать тесты, которые обязан
прогнать. Отказ при этом **молчаливый**: пропущенный тест выглядит как зелёный
прогон — ровно тот класс, который вердикт судьи по эпику HATS-1252 назвал
«иммунная система скомпрометирована как целое» [9]. Поэтому порядок **T3 → T2**, а
не параллельно: предусловие 1 покупается работой по D5, предусловие 2 — тем же
обходом графа, что и линт D5. Строить отбор раньше — значит строить его на графе,
который врёт.
...
вручную. **Свёртка поверхностей (HATS-1826) сняла четыре дистрибуции**, поэтому
знаменатель здесь теперь четыре, а не пять; числитель мерян методом, который эта
правка не воспроизводит, и перемеряется слайсом способности 6, а не тут.
...
**Вторая область планировалась как `consent`**: только она прогоняет механику
«область пересекает границу дистрибуций» (D10) и случай неимпортного связывания через
console-script и подпроцесс, которого импортный линт не видит вовсе.

**Что изменил выбор 2026-08-25.** Второй областью стала `surfaces` (HATS-1826), и не
по плану, а по живому следствию: четыре поверхности были дистрибуциями, которых нет на
PyPI, и `self_heal` ставил их по имени. Порядок D6 ведётся «по живому следствию, а не по
цене» — дефект, который воспроизвёлся трижды (agy → codex → opencode), опережает область,
выбранную за полноту механики. `consent` остаётся следующим кандидатом, и по той же
причине, что и была: его D10-механика по-прежнему не прогнана ни одной областью.

**Гейты пилота.** Пилотом (`pipeline`) не достигнуты; вторая область взяла первый из
них, чего пилот не смог:

| Метрика                                           | Цель                                                  | `pipeline` | `surfaces` |
| ------------------------------------------------- | ----------------------------------------------------- | ---------- | ---------- |
| Глубокие входы в область (импорт мимо `__init__`) | **0** — иначе граница не держится; это её определение | 3          | **0**      |
| Модули области в нетривиальном SCC                | **0** (для `pipeline` требует разреза цикла)          | 0          | 0          |
| Тесты области в собранном колесе                  | **отсутствуют** (D11)                                 | нет        | нет        |
| `python -m ai_hats --help`                        | **≤ 0.25 с** против базовой 0.20                      | 0.17 с     | 0.17 с     |

Ноль у `surfaces` куплен не аккуратностью, а **наследством**: у области было **два**
глубоких входа против 107 у пилота, потому что четыре из пяти поверхностей приехали
из отдельных дистрибуций, которые интегратор не импортировал ни разу — он резолвил их
через группу entry-point. Оба входа сняты в самой карточке. Это и есть цена, которую
D12 просил измерить: разрыв между 2 и 107 объясняет, почему гейт «глубокие входы = 0»
берётся за слайс в одной области и не берётся за эпик в другой, и почему **число
унаследованных входов — первое, что меряют о кандидате**, раньше его размера.
...
добавляется. Рецепт пилота (`pipeline`) сдан и лежит в [15].
...
**Кто обязан следить.** У каждого поддерева в таблице владелец назван, и именно он
**объявляет политику истечения** для своего корня — срок, условие и то, что
считается доказательством смерти. Механизм сверки и удаления при этом один на
всех, `ai_hats/reclaim/` (C15), и политику он **получает** объектом, а не
импортирует владельца. Поддерева без названного владельца в целевом состоянии
быть не может; строка закрыта тогда, когда у неё есть и владелец, и тест,
доказывающий, что живой артефакт по возрасту не сносится.
...
- [14] `.agent/ai-hats/tracker/backlog/tasks/HATS-1782/task.yaml` — построчная
  ревизия колонки «Проверяющий тест»: привести все 20 гейтов к отрицательному
  универсалу или сознательно оставить свойство поведения, записав почему.
- [15] `docs/adr/attachments/area-extraction-notes.md` — рабочий контекст эпика
  HATS-1586, которым агенты обмениваются перед правкой области: соглашения,
  таксономия «гейт зелёный, свойства нет» и измеренный рецепт пилота `pipeline`,
  которого требует D12. Не пользовательская документация — приложение к этому ADR.
