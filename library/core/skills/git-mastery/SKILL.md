...
## Hook Self-Heal (post-merge / post-checkout, HATS-593)

`.githooks/` is generated by composition and is **not** tracked in git, so it
drifts after `git merge` / `git pull` / `git checkout` until someone re-runs
`ai-hats self init`. This skill ships `self-heal-hooks.sh` as both a
**post-merge** and **post-checkout** hook. At the moment drift is introduced
they run `ai-hats self sync-hooks`, which re-materializes only the git-hook
surface (cheap, idempotent no-op when already in sync).

- **Fail-open:** the hook always `exit 0` — a heal failure never aborts your
  merge / checkout. On failure it prints a loud warning ("run `ai-hats self
  init`"), never degrades silently.
- **post-checkout** acts only on a **branch checkout** (not a per-file
  `git checkout -- path`).
- **Bootstrap:** a fresh clone still needs one initial `ai-hats self init` to
  install hooks at all; they self-maintain after that.

You do not invoke these hooks manually — they fire from git. If you ever see
a "hooks corrupt" message on push, the gate's fail-closed dispatcher caught a
missing managed hook: run `ai-hats self init` to repair.
