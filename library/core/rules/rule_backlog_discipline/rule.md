...
1. **CLI-only.** All backlog operations via `ai-hats task` CLI (`task ...`, `task hyp ...`, `task proposal ...`). Never read or edit `<ai_hats_dir>/tracker/backlog/**` or `<ai_hats_dir>/tracker/hypotheses/**` directly.
