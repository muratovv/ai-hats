...
| `tests/sessions/`    | **session-integration** | CLI-команда собирает и гоняет свой пайплайн по-настоящему; runner-граница (`WrapRunner` / `SubAgentRunner` / `SessionReviewRunner` / `subprocess.Popen`) застаблена через `conftest.py::mock_runners`. Никакого реального Claude/shell subprocess. Звалось `tests/pipeline/`, хотя предмет здесь — не область: `conftest.py` патчит `runtime`, `retro` и `_bootstrap`, а pipeline-модули отсюда импортируются лишь постольку, поскольку сессия их запускает (HATS-1783). |
...
Default `pytest` прогоняет всё, что перечислено в `testpaths`: `tests/`, тесты областей (`src/ai_hats/<area>/tests/`) и тесты пакетов
(`packages/*/tests/`). CI прогоняет с
