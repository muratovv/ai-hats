...
def test_load_linked_context_happy_path_and_ordering(tmp_path: Path) -> None:
    ...
    body = _runner(project_dir)._load_linked_context("HATS-902")
...
def test_load_linked_context_unknown_ticket_returns_empty(tmp_path: Path) -> None:
    project_dir = tmp_path / "proj"
    project_dir.mkdir()
    assert _runner(project_dir)._load_linked_context("HATS-999") == ""
    assert _runner(project_dir)._load_linked_context("") == ""
