...
def _write_profile(path, *, policy="smart", min_turns=5, min_tool_calls=10, mode="programmatic"):
...
class TestPolicyOff:
    def test_skip(self, tmp_path):
        ...
        _write_profile(profile, policy="off")
        _write_metrics(metrics)

        action, _ = should_run(profile, metrics)
        assert action == "skip"
...
class TestPolicyAlways:
    def test_run(self, tmp_path):
        profile = tmp_path / "profile.json"
        metrics = tmp_path / "metrics.json"
        _write_profile(profile, policy="always")
...
class TestPolicySmart:
    def test_skip_both_below(self, tmp_path):
        profile = tmp_path / "profile.json"
        metrics = tmp_path / "metrics.json"
        _write_profile(profile, min_turns=5, min_tool_calls=10)
...
class TestPolicyHint:
    def test_hint_when_threshold_met(self, tmp_path):
        profile = tmp_path / "profile.json"
        metrics = tmp_path / "metrics.json"
        _write_profile(profile, policy="hint", min_turns=5)
    ...
    def test_skip_when_below(self, tmp_path):
        profile = tmp_path / "profile.json"
        metrics = tmp_path / "metrics.json"
        _write_profile(profile, policy="hint", min_turns=5, min_tool_calls=10)
...
class TestEdgeCases:
    def test_missing_profile_uses_defaults(self, tmp_path):
        """No profile.json → defaults (smart, threshold 5/10)."""
    ...
    def test_missing_metrics_skip(self, tmp_path):
        ...
        _write_profile(profile, policy="smart")
