...
def test_gc_lock_path_points_at_versions_dotlock(tmp_path: Path) -> None:
    assert gc_lock_path(ProjectLayout.at(tmp_path).versions) == (
...
def test_acquire_creates_parent_dir_and_releases(tmp_path: Path) -> None:
    ...
    assert not gc_lock_path(ProjectLayout.at(tmp_path).versions).parent.exists()
    with versions_lock(ProjectLayout.at(tmp_path).versions, timeout=1.0):
        assert gc_lock_path(ProjectLayout.at(tmp_path).versions).parent.is_dir()
...
def test_contention_raises_version_lock_error(tmp_path: Path) -> None:
    ...
    try:
        with pytest.raises(VersionLockError) as exc:
            with versions_lock(ProjectLayout.at(tmp_path).versions, timeout=0.2):
                pass
