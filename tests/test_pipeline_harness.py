...
def test_old_sessions_pruned(tmp_path: Path, monkeypatch):
    ...
    h = PipelineHarness("execute", ProjectLayout.at(tmp_path), session_id="new-sid")
    with h:
...
def test_namespace_per_pipeline_isolated(tmp_path: Path):
    bare = PipelineHarness("human", ProjectLayout.at(tmp_path))
    execute = PipelineHarness("execute", ProjectLayout.at(tmp_path))
    with bare, execute:
...
def test_parallel_runs_disjoint(tmp_path: Path):
    ...
    h1 = PipelineHarness("p", ProjectLayout.at(tmp_path), session_id="sid1")
    h2 = PipelineHarness("p", ProjectLayout.at(tmp_path), session_id="sid2")
    with h1, h2:
...
def test_explicit_session_id_passthrough(tmp_path: Path):
    h = PipelineHarness("p", ProjectLayout.at(tmp_path), session_id="MYSID")
    assert h.session_id == "MYSID"
    assert h.namespace.name == "MYSID"


def test_auto_session_id_format(tmp_path: Path):
    h = PipelineHarness("p", ProjectLayout.at(tmp_path))
...
def test_materialize_prompt_writes_file(tmp_path: Path):
    with PipelineHarness("execute", ProjectLayout.at(tmp_path)) as h:
...
def test_run_loads_yaml_and_executes(tmp_path: Path):
    ...
    with patch(
        ...
        with PipelineHarness("reflect-session", ProjectLayout.at(tmp_path)) as h:
            final = h.run(
                {
                    "session_id": "x-1",
...
def test_harness_no_trace_when_env_unset(tmp_path: Path, monkeypatch):
    ...
    h = PipelineHarness("execute", ProjectLayout.at(tmp_path))
...
def test_harness_enables_trace_in_default_location(tmp_path: Path, monkeypatch):
    ...
    h = PipelineHarness("bare", ProjectLayout.at(tmp_path))
...
def test_harness_respects_ai_hats_dir_override(tmp_path: Path, monkeypatch):
    """AI_HATS_DIR cascades into the layout the entry resolves (scoped trust)."""
    ...
    layout = ProjectLayout.compute(tmp_path, {ENV_AI_HATS_DIR: str(custom)})
    h = PipelineHarness("reflect-all", layout)
    assert h.trace_path is not None
...
def test_harness_writes_trace_file_on_run(tmp_path: Path, monkeypatch):
    ...
    with patch(
        ...
        MockRunner.return_value.run.return_value = fake_path
        with PipelineHarness("reflect-session", ProjectLayout.at(tmp_path)) as h:
            h.run({"session_id": "x-1", "layout": ProjectLayout.at(tmp_path)})
...
def test_harness_loads_user_steps_on_enter(
    ...
    with PipelineHarness("any-name", ProjectLayout.at(tmp_path)):
...
def test_harness_user_step_collision_aborts_before_namespace_setup(
    ...
    h = PipelineHarness("any-name", ProjectLayout.at(tmp_path), session_id="ssid")
