...
# ---- HATS-378 Phase 4: pipeline_metrics.json aggregation ----


def test_pipeline_metrics_written_on_clean_exit(tmp_path: Path):
    h = PipelineHarness("any", tmp_path, session_id="sid-clean")
    with h:
        pass
    pm = h.namespace / "pipeline_metrics.json"
    assert pm.exists()
    data = json.loads(pm.read_text())
    assert data["pipeline"] == "any"
    assert data["session_id"] == "sid-clean"
...
def test_pipeline_metrics_records_zero_output_incident(tmp_path: Path):
    from ai_hats.harness.errors import HarnessZeroOutputError

    h = PipelineHarness("any", tmp_path, session_id="sid-zo")
    with pytest.raises(HarnessZeroOutputError):
        with h:
            raise HarnessZeroOutputError("sub-1", "diag")
    data = json.loads((h.namespace / "pipeline_metrics.json").read_text())
    assert data["incidents"]["silent_zero_output_incidents"] == 1
    assert data["incidents"]["harness_timeout_incidents"] == 0


def test_pipeline_metrics_records_timeout_incident(tmp_path: Path):
    from ai_hats.harness.errors import HarnessTimeoutError

    h = PipelineHarness("any", tmp_path, session_id="sid-to")
    with pytest.raises(HarnessTimeoutError):
        with h:
            raise HarnessTimeoutError("sub-1", "diag")
    data = json.loads((h.namespace / "pipeline_metrics.json").read_text())
    assert data["incidents"]["harness_timeout_incidents"] == 1
    assert data["incidents"]["silent_zero_output_incidents"] == 0


def test_pipeline_metrics_ignores_unrelated_exceptions(tmp_path: Path):
    """ValueError or any non-HarnessReliabilityError must not bump counters."""
    h = PipelineHarness("any", tmp_path, session_id="sid-other")
...
def test_pipeline_metrics_does_not_mask_underlying_exception(tmp_path: Path):
    """If pipeline_metrics write fails, the original exception still bubbles
    (best-effort observability — never fatal)."""
    from ai_hats.harness.errors import HarnessTimeoutError

    h = PipelineHarness("any", tmp_path, session_id="sid-mask")
    # Sabotage namespace by replacing it with a file so mkdir/write fails.
    h.namespace.parent.mkdir(parents=True, exist_ok=True)
    h.namespace.touch()  # exists as a file, mkdir(exist_ok=True) is fine but write_text would fail

    with pytest.raises(HarnessTimeoutError):
        # NB: __enter__ would have failed if namespace was a file (mkdir),
        # so this exercises only the __exit__ best-effort branch. We bypass
        # __enter__ for the test.
        h.__exit__(
            HarnessTimeoutError,
            HarnessTimeoutError("sub", "diag"),
            None,
        )
        raise HarnessTimeoutError("sub", "diag")
