...
def _claude(result, project, skills_dir):
    ...
    return composed_rows(result, skills_dir).get(EVENT, [])


def _cline(result, project, skills_dir):
    from ai_hats.surfaces.cline.runtime_hooks import _manifest

    return _manifest(project, result, SESSION_ID, skills_dir=skills_dir)["hooks"].get(EVENT, [])
...
def _opencode(result, project, skills_dir):
    ...
    return _manifest(project, result, SESSION_ID, skills_dir=skills_dir, permission_rules=[])[
        "hooks"
    ].get(EVENT, [])
...
def _agy(result, project, _skills_dir):
    ...
    return AgySurface()._hooks_manifest(project, result, SESSION_ID).get(EVENT, [])
