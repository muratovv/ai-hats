...
def _extract_session_block_body(text: str) -> str:
    ...
    Mirrors how ``ClaudeSurface.plan`` writes the context file:
    it inserts ``f"{START}\\n{prompt_content}\\n{END}"`` where
    ``prompt_content`` is the plan's ``prompt.text``. So stripping the
    markers + edges yields exactly the materialized text.
    """
