...
def test_reflect_session_foreground_happy(
    ...
    res = CliRunner().invoke(
        main,
        ["reflect", "session", "--session", "20260101-000000-1"],
    )
    assert res.exit_code == 0, res.output
    assert mock_runners["session_review_calls"] == [("20260101-000000-1", 1)]
...
def test_reflect_session_foreground_max_retries(
    ...
    res = CliRunner().invoke(
        main,
        ["reflect", "session", "--session", "x-1", "--max-retries", "3"],
    )
