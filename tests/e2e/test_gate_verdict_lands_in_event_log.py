...
flow:   an operator's session has a gate that refuses one path; while the
        session runs, its own events.jsonl says which gate refused which call
cmds:
...
expect: one gate_verdict line per call, appended by the hook process into the
        ...
        and the verdict claude received is unchanged by the recording
...
def test_each_verdict_lands_in_the_sessions_event_log(session) -> None:
    ...
    # POSITIVE CONTROL: the chain really refused, then really allowed — and the
    ...
    assert "gate verdict not recorded" not in refused.stderr + allowed.stderr

    assert session.log.is_file(), f"no {EVENT_LOG_JSONL} beside the session's artifacts"
    events = list(read_events(session.log))
    assert [type(e) for e in events] == [GateVerdict, GateVerdict], events
    deny, allow = events
