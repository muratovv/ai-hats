...
def test_e2e_update_banner_fires_for_non_editable_install(tmp_path: Path) -> None:
    ...
    for i in range(LAG_COMMITS):
        subprocess.run(
            [
                ...
                "--allow-empty",
                "-m",
                f"HATS-458 e2e: synthetic ahead commit {i + 1}",
            ],
            check=True,
        )
    ...
    subprocess.run(
        ["git", "-C", str(fake_remote), "update-ref", "refs/heads/master", ahead_sha],
        check=True,
    )
    ...
    assert cache_data["installed_sha"] and installed_sha.startswith(cache_data["installed_sha"]), (
    ...
    assert cache_data["latest_sha"] == ahead_sha, (
        f"cache latest_sha mismatch: expected {ahead_sha!r}, got {cache_data['latest_sha']!r}"
    )
    ...
    assert cache_data["ahead"] == 0, (
        f"cache ahead mismatch: expected 0, got {cache_data['ahead']!r} (cache: {cache_data})"
    )
    ...
    assert "ai-hats update available" in banner.stderr, f"banner missing; stderr:\n{banner.stderr}"
