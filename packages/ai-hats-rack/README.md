...
2. `AI_HATS_DIR` environment override (points to `<ai_hats_dir>`, cards under `<ai_hats_dir>/tracker/backlog/tasks`). If `AI_HATS_PROJECT_DIR` is set and does not match the project directory containing `<ai_hats_dir>`, `resolve_root` raises `ForeignProjectPinError` (exit code 1, `foreign_project_pin`) detailing both paths and `ai_hats_dir`.
3. Walk-up from CWD to the nearest ancestor holding `.agent/` or `ai-hats.yaml` (K2, HATS-197 heir); from inside a linked task worktree (neither marker present) a pure-filesystem gitlink hop resolves the main checkout instead (HATS-1038 C2).
