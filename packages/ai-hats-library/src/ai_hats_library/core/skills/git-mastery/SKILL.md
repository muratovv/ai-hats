...
ai_hats:
  ...
  git_hooks:
    ...
    # session-start managed-hook net (WrapRunner._resync_managed_hooks), not
    # a git-event hook.
license: MIT
---
...
on every ai-hats launch it re-materializes any drifted managed hook surface —
git and runtime (`.claude/settings.json` plus the scripts under the project's
`<ai_hats_dir>/library/hooks/`) — and prints a one-line note of
