...
`verification_protocol`-anchored ones.

## Strict vs loose protocols in the wild

- **HYP-016 — strict three-line protocol:**
...
- **HYP-017 — loose paragraph protocol:** "one paragraph
...
## ✓ Good: protocol-anchored verdict

HYP-016 carries the strict three-line `verification_protocol`. Session
evidence: a library-curation child task transitioned plan→execute and filed
HYP-NNN with a `verification_protocol` field. Verdict:

```bash
rack hyp append-verdict HYP-016 \
  --session-id "$SID" \
  --verdict confirmed \
  --evidence "CRITERION: ≥3 of next 4 child tasks under the epic land with either a companion HYP that has verification_protocol filled, or a work_log entry no behavior change — pure refactor: <reason>.
OBSERVED: PROJ-543 plan→execute transition; HYP-019 filed with verification_protocol set; commit body references PROJ-543/HYP-019.
...
`evidence: "saw PROJ-543 file a HYP, looks good"`. Wrong — the
