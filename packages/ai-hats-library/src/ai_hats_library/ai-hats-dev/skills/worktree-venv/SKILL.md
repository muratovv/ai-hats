...
ai_hats:
  # hook-carrier skill. The lifecycle bundle spawns this script in
  # place from the skill dir after `git worktree add` (a migration retired the
...
design) tests the wrong source and the guard blocks the
first commit. The paired half is the `git-mastery` smoke hook preferring
`<git-toplevel>/.venv/bin/pytest`; neither half works alone.
...
- **Warn-continue** (ADR-0012 D3/D7). Every declining path exits 0: no root
  ...
  status quo before this hook, never worse.
