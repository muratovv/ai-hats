...
- Reference a backlog ID in the subject when applicable, e.g.
  `feat(launcher): venv-first wrapper`.
...
runnable and unlisted (`tmp-sweep` was). To add one, define
...
- `make lint` — `ruff check .` plus `ruff format --check` over that same `.`
  (two scopes could not stay equal by convention).
- `make e2e` — the maintainer tier, and the very stage the master pre-push gate
  runs (one selection, not a copy), bounded by timeout (default 3600s).
...
- git hooks — `.githooks/<event>` dispatchers only (no `*.d/`
...
- provider runtime hooks — per-session `settings.json` pointing directly into the
  session tree plugin skills.

**Session-tree isolation.** Managed runtime hooks live in the
...
**The session-start drift net is retired.** It existed to police the
...
The **stub** is deliberately **fail-open**: when the interpreter
...
Once the entry point IS executing, the answer flips. A gate ai-hats
...
git gates are NOT re-checked at session start (the project is written
...
2026-08-26. Check the answer with `ai-hats list tokens <role>` before and after:
...
- **API keys, bearer tokens, `.env` files** — the hook will block these
  ...
  — the override is a documented act, not a hidden one.
