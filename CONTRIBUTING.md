...
### Managed-hook self-heal (session start)

The managed-hook surfaces are **generated by composition** and (except
`.githooks/`'s tracking) regenerated on disk only by `ai-hats self init` /
`set-role` / `self update` — so they go stale between those runs:

- git hooks — `.githooks/` dispatcher + `*.d/` scripts + `.ai-hats-manifest`;
- provider runtime hooks — `.claude/settings.json` managed entries + the
  `library/hooks/` scripts they point at;
- worktree hooks — `library/wt-hooks/` scripts.

**One drift net heals all three, at session start (HATS-593 → HATS-833).**
`WrapRunner._resync_managed_hooks` calls `Assembler.sync_hooks` once per launch
(reusing the session's composition). It is drift-gated and idempotent — a no-op
when everything matches the composed source — and re-materializes / re-wires any
surface that drifted. It is **not** a full `self init` (no role/skill/prompt
recompose). On a heal it prints a one-line **startup note** naming each healed
hook and why (`materialized` / `updated (content drift)` / `re-wired`); a clean
start is silent.

There is intentionally **no** `ai-hats self sync-hooks` command and **no**
post-merge / post-checkout git hook anymore — HATS-833 consolidated all healing
to session start. **Trade-off:** a `git pull` + `git commit` with no ai-hats
launch in between can run stale git hooks until the next launch heals them; the
dispatcher backstop still fails closed on a *missing* gate (below). Launch
ai-hats (or `ai-hats self init`) to refresh sooner.

**Failure policy.** The session-start net is **fail-open** — a heal failure
never blocks the launch; it surfaces a **loud, actionable warning** instead of
degrading silently:

- composition raises / write fails → warn, leave the prior hooks intact (never
  half-written), launch proceeds.
- **version skew** — if the installed binary is *behind* upstream master, the net
  refuses to materialize blind, recommends `ai-hats self update`, and the warning
  names the drift left unhealed (status `version-skew`).
...
**Bootstrap caveat (inherent, not a bug).** Hooks are ai-hats-managed, so a
**fresh clone still needs one initial `ai-hats self init`** to install any hooks
at all. After that the session-start net maintains them on each launch.
