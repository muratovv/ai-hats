...
- **A HITL claude session's log says the quota limit is near, and every record Claude writes has a reading** (HATS-1992). A PTY session has no SDK stream and its transcript carries no quota state, so the one place it sees its rate-limit windows — the status line's stdin payload, `rate_limits.<window>.{used_percentage, resets_at}` from the first API response on — is now a producer: every HITL claude session's settings name a `statusLine` command — the same hook dispatcher every gate entry runs, observing a render the way it observes Claude's permission notification, so the resident dispatcher answers over its socket — that appends `Notice(approaching_limit, source: claude/statusline)` once per window per reset when a window reaches `AI_HATS_APPROACHING_LIMIT_PERCENT` (a budget, 80 by default, on the env reference page), then runs the person's own status-line command with the same payload, because `--settings` replaces that slot and a bar is not a gate. On the transcript side the replay of 3124 transcripts found what the reader lost: a `fallback` content block (the API rerouting a call) reported as drift instead of `model_switched`; 94 `malformed-json` notices and six dropped records from `str.splitlines` cutting at a raw U+2028; and 132 hooks of ours that exited non-zero or timed out, recorded nowhere — now `surface_warning`. The 87k `attachment` records over 31 subtypes and the 19 bookkeeping types are each named with the reason they say nothing, so an unmeasured subtype is drift rather than one more thing that happens to be quiet; the triage lives in the live-log surface survey, and `scripts/replay_claude_corpus.py` is the per-release re-measurement (counts only, never a transcript's content). `--include-hook-events` is not passed: the stream's hook messages carry no `tool_use_id`, the transcript's attachments do. `ai-hats-observe` 0.13.0.

## [0.15.1] - 2026-09-20

### Changed

- **The smoke subset is asked before the merge, not after it** (HATS-1673). `unit` and `integration` never touch the installed binary; `merge-smoke`, the curated subset of `tests/e2e`, is the one stage that checks ai-hats still launches, and it stood on `->done` alone, after the branch was already in master. It now sits on `->review` and `->merge` as well, and the two keep demanding the same set, so one run still clears both; `make done-gate` after a green `review-gate` pays for `integration` and `e2e-default` only. A run's worker count now defaults to `-n3` (the cap of eight starved a laptop the agent shares with its supervisor), and a `-n` already in the caller's `PYTEST_ADDOPTS` wins. Measured on a clean tree, 14 cores, at the `-n3` default: `merge-smoke` 9.7 s against `unit` 94 s, and the whole `review-gate` from scratch, with the zones this diff itself touches (`gates`, `rack`), 260 s; at `-n8` the same run took 183 s (`merge-smoke` 6.6 s, `unit` 62 s).

- **A card's close no longer waits on master's own CI verdict** (HATS-1991). `master-ci` sat on `->done` as "the same question about the base the card lands on" — but a card can neither earn nor fix that verdict, so a finished card sat in review behind a red run it had no part in, with no honest exit: the one hatch was for the card that fixes master, not for somebody else's red. The stage leaves every card gate — `make done-gate` after a green `review-gate` now pays for `integration`, `merge-smoke` and `e2e-default`, nothing on the network — and stays as a stage run by hand. The alarm it was built for moves to where the one who can act stands: the pre-push hook prints master's last verdict before a push to master (`scripts/check_master_ci.py --notice`) and never blocks on it, since the push is how master gets fixed. `AI_HATS_RED_MASTER_ACK` is retired with the gate that needed it.

- **A session is one value — planned, applied, launched — on every surface, and the dry-run is a rendering of that value** (HATS-1971 → HATS-1981, HATS-1982; ADR-0036). Until now a session was *built*: each surface's `_build_<category>_<mode>` handlers wrote through a materialization port as they went, the launch argv was assembled once more at spawn, and `--dry-run` swapped in a recording port and reported what the recording saw — which is how a command the report showed could differ from the one that ran (HATS-1552) and a hook the prompt never mentions stayed invisible (HATS-1955). Now `Surface.plan` turns the composition into a frozen `MaterializationPlan` — the entries, the child's environment, the launch, the prompt — reading nothing from disk; the person's configuration home (`~/.codex`, `~/.config/opencode`) and the machine's facts arrive as an enumerated `Host` input, and two plannings of one input compare with `==` (the stage's exit condition, held by `tests/test_stage_acceptance_plan.py` on five surfaces × two run modes). `apply` writes the plan idempotently — a second application on the same root writes nothing — and `launch` builds the argv or the SDK option document from the plan alone, so the record `role_materialization.json` holds, `audit.md` cites and `--dry-run` prints are projections of the one value that ran. Visible changes to the report: `--dry-run` and `ai-hats agent --dry-run` render the plan's record — each write with its `source`, `size` and `digest`, the check bindings with where each `runs_from` and whether it is `planned`, a `composition` section with the prompt blocks, skills and hooks by full name — and the `duplicates` / `escapes` lists are gone, because a duplicate target is now a planning refusal and an escape from the root has to be declared by the planner. `--dry-run-json` is that record; `--materialize` applies it to a fixed root and adds what `Applied` reports. **Breaking for a surface plugin** (the `ai_hats.providers` entry point): `plan` is required, and `build_session_prompt`, `build_session_artifacts`, `build_category_artifact`, `handles_artifact_categories`, `materialize_runtime_skills` and `describe_automate_launch` are gone from `Surface`, as are `result`, `artifacts`, `task` and `ticket_id` from `SubagentEngine.run` (it takes `launched` and `brief`); a registered surface without a planner is refused by name at session start and in a dry-run instead of being delivered an empty session through the legacy branch. The pre-1.0 exemption of `docs/RELEASING.md` is claimed: every consumer of the removed members is one of the five in-tree surfaces, all migrated in this card, and no out-of-tree surface is known.

### Added

- **The README is a pitch again, and a page says which harness supports what** (HATS-1950). It promised two harnesses of five, never said the word consent, and buried the loop mid-page; it is now 140 lines built on four claims — role-based, multi-harness, framework, behavior feedback — each naming things rather than adjectives. `docs/surfaces.md` is new: a per-capability matrix over all five harnesses, read out of the classes themselves — what a hook gate may utter (`Dialect`), whether the audit is parsed from a real session log or falls back to the trace, how sub-agents run, whether consent wrappers work. `agy` is the one without consent wrappers, and a role that declares a consent gate there fails before the agent starts rather than running unwrapped. Recovery and the update banner moved out of the README into `how-to-advanced.md` §7-§8, and the README is now exempt from the numbered-reference convention: a survey of 22 well-regarded OSS READMEs found none using one.

- **A `tech-writer` role, for editing prose rather than writing it** (HATS-1890). Its default move on any text — the user's draft or its own — is a critique, not a rewrite: name the one thing the text is for, name what is said twice, check that every section hands the reader something. Three skills carry it, and the two reusable ones are context-free today so another project can compose them by name: `prose-review` (the report's shape and the per-language style references, Russian and English), `claim-evidence-check` (every checkable statement becomes a row with a source or an honest `unverified` / `contradicted` mark, plus a stdlib link checker), and `article-outline` (the Diátaxis quadrant, audience and style forks settled before a word of body text). Portability was checked by hand, with a positive control: the same pattern that finds nothing in the two reusable skills finds `plan.md` in `article-outline`, where the ai-hats glue deliberately lives. No gate runs that pattern, so this is a statement about today's tree, not an invariant. What the role knows about editing is deliberately not in the always-on injection — conduct rules with worked examples sit in `prose-review/references/editor-conduct.md`, and the FSM is left to `hatrack` and `rule_backlog_discipline` rather than restated. The role reviewed the prose that defines it before shipping, and its own findings are folded in: a style reference that filed деепричастия under причастия, a link checker that reported `checked 0 link(s)` over a corpus whose one citation it could not see, and a conduct example that marked a claim `unverified` while citing the source contradicting it.

### Added

- **A session leaves its canonical events beside the artifacts projected from them** (HATS-1966). `events.jsonl` is written next to `audit.md` and `usage.json` by a `write_event_log` step sitting beside `compute_usage` in both finalize pipelines. Purely additive — the existing artifacts are byte-for-byte what they were, and a surface with no canonical reading of its own transcript writes no such file rather than an empty one, since an empty log would claim a session that produced nothing. The record is complete rather than a summary: an audit rendered from the log equals the audit rendered from the transcript that produced it, checked over 300 real transcripts with 297 of them carrying both turns and token totals. That equality is what makes the log worth keeping — every later consumer reads the record instead of re-deriving it from prose.

- **`events.jsonl` is written while the session runs, and carries every gate's verdict** (HATS-1967). The finalize step that wrote it after the fact is gone; the session's own `EventLogWriter` follows the surface's record from before the surface starts to after it exits, appending each canonical event as it appears, so `tail -f` on the file shows the run as it happens and nothing rewrites it at the end. The same file now records what the composed hook chain concluded on every tool call — `gate_verdict` lines naming the point (`before_tool` / `after_tool` / `at_stop`), the decision (`allow` / `deny` / `ask`), the deciding hook and its reason — appended by the dispatcher itself, in the wrapper process or in a hook process of its own, and Claude's own `stop_hook_summary` reads as the verdict at the stop instead of being silenced. Two producers appending to one file is why every line is now exactly one `write(2)`. Sub-agents get the same treatment: the runner mints the surface's session id up front and hands it to the SDK, so the record can be followed before the surface reports one. A session the surface gave no id (claude on `--resume`) is not followed rather than followed by time alone, and says so in its trace.

### Fixed

- **`reflect role` hands the auditor the composition the session runs, rule bodies included** (HATS-2005). Every `rules/<name>.md` in the composed dir was 0 bytes: the writer read `ResolvedComponent.injection`, which the composer stopped filling when it stopped eager-loading `rule.md`, and only the skills branch had been moved to `source_path`. The rules branch now reads `read_rule_body(source_path)`, the same reader the prompt uses. The audit also composed the built-in role alone, on purpose, so a user-global or project override (a `personal-workflow` trait with three rules, in the case that surfaced this) was invisible to the very check that asks about interference with the user's context. The target is now composed through `build_preview_payload`, the read-only path `config show-prompt --role` takes, and `manifest.yaml` carries `identity` and the plan's `trace` (`term` / `brought_by` / `removed_by`, one shape with the session record), so the auditor can route a finding to the library or to the customization by the layer that brought the term. `role-coherence-protocol` § Inputs and § Step 2 name the new input; the one documented no-overlay `composer.compose` site outside the facade is gone with it.

- **The launcher run from a subdirectory answers for the project, not the subdirectory** (HATS-2002). `scripts/ai-hats-launcher` took `$(pwd)` as the project root (with the linked-worktree hop but no marker walk-up), so `ai-hats <cmd>` from `<project>/sub/` bootstrapped a stray `sub/.agent/ai-hats/.venv` from upstream and triaged it as broken, while the python side (`resolve_root`) had walked up to the project all along. The launcher now mirrors that order — worktree hop, else the nearest dir above cwd carrying `.agent/` or `ai-hats.yaml`, else cwd (a bare dir is still a fresh `self init`) — and the parity e2e holds the two together. Root resolution is what `LAUNCHER_CONTRACT` tracks, so it moves 2 → 3; a host launcher behind it is named by `self update` with the refresh. Found by the HATS-1998 review.

- **A local channel with no source is healed by `self update`, not handed to uv** (HATS-1998). `harness: {channel: local}` without a `path` means "editable install of the project root", which is a Python project only for the ai-hats checkout itself; the wizard writes exactly that when `local` is chosen on a host whose ai-hats is a git install (no editable source to detect), and every heal contour looked past it — the launcher heals only a broken venv, `_read_harness` degrades only an unparseable yaml, and the triage had no harness row — so `self update` ran `uv pip install -e <project root>` and printed uv's refusal under an all-green triage. The local source is now resolved once (explicit `path`, else the editable install the process runs from — its PEP 610 url decoded, uv percent-encodes it — else the root) and judged by identity, a `pyproject.toml` naming `ai-hats`: the first cut asked whether uv would accept the dir, and an adversarial review showed that certifying a consumer's own pyproject and installing the consumer into the tool venv, reported as success. Triage carries a `harness` row (`--check` exits 1 on it); a source that is not ai-hats degrades that run to edge with a warning naming the origin (`harness.path`, `AI_HATS_INIT_SRC`, the editable install, the project root) and the matching fix, the yaml stays the user's, and the launcher's recreate falls back to the pip target the same way — but an editable install is refused, never replaced from edge. An edge `repo` given as a path (or a local `AI_HATS_REPO_URL`) is judged by the same predicate before uv is asked, and a missing path is named as missing, not as offline. `_run_editable_update` takes the resolved absolute path; the cwd-relative `.` is gone. Found alongside: the launcher's foreign-pin guard (HATS-944) re-pinned `AI_HATS_PROJECT_DIR` and `AI_HATS_VENV` to the cwd project but left the session envelope, and the python entry resolves the project from the envelope (ADR-0024), so a command run from another project inside an agent session still answered for the session's project — the launcher now drops `AI_HATS_SESSION_IDENTITY`, `AI_HATS_SESSION_ID` and `AI_HATS_ROLE` with the pin. A direct `python -m ai_hats` from a foreign cwd keeps envelope-wins by design. The same review found a pre-existing crash the new road led into: the launcher's heal recreates `versions/<sha>` without its `.complete` sentinel, so the managed update read the target as crash residue and `rmtree`'d the venv it was running in — now the live prefix is handed in from the entry point, verified and adopted, never rebuilt under its own interpreter. Two of these live in the host launcher (what it heals from, and what a command resolves to under a foreign pin), so `LAUNCHER_CONTRACT` moves 1 → 2: `self update` on a host whose launcher predates this says so and names the refresh, `curl -sSL https://github.com/muratovv/ai-hats/raw/master/scripts/install-launcher.sh | bash`.

- **A session launched from a worktree is measured again** (HATS-1986). Claude keys a session's transcript by the directory the surface *ran in*; ai-hats's finalize looked for it under the project root — the main checkout, for a session launched from a task worktree — so `make_audit` and `compute_usage` reported `no-structured-transcript`, `audit.md` fell back to the PTY trace, `usage.json` was never written and the auto-retro skipped the session as unmeasured, while the live `events.jsonl` beside them was complete. Every tracked task here runs from a worktree, so this was most sessions. The sub-agent path had the mirror defect: it repaired the resolver by re-rooting the *whole* layout on the sub-agent's checkout, unresolved — so a `/var/folders` discard worktree missed its `/private/var/folders` record, and the retro decision was logged into a tree that was about to be deleted. The fix is one value on the object every consumer already holds: `ProjectLayout.cwd`, where the process stands as distinct from the root it resolved, stamped once at the entry point and copied with `with_cwd` for a sub-agent's worktree. The transcript resolvers, the live event-log writer and the launch record read it; the retro log, the rack workspace and the cache keep reading the root. Rack's `caller_cwd` in the quorum sweep is that cwd too, and `role_materialization.json` now records where the surface actually ran. A launch from a subdirectory of the main checkout is found for the same reason. The resolver's first parameter is renamed from `project_dir` to `cwd`, because the old name is what invited passing the root.
- **A tracker id in `src/` is refused, and the code surface is at zero** (HATS-1920). `ticket-ids` judged the library's prose and the docs a reader opens; `src/` was left out on the argument that removing an id from code is a rewrite rather than a deletion. It is — and four fifths of it was prose anyway. Measured by tokenizer rather than regex, because an f-string is not a STRING token in 3.12+ and a naive parse loses it: **984 ids in docstrings, 247 in comments, 29 in message strings, 0 in code** — 1260 across 200 files, all gone. The gate's corpus now covers **all** of `src/`, not `src/**/*.py`: a suffix list would have missed the `.sh`, the `.mjs` and the two extensionless hook scripts there, three of which carried ids. Two independent measurements of the starting tree agreed to the id — the gate's line regex and the tokenizer census both said 1260 — which is the thing a zero alone cannot establish.

  **`TODO(<id>)` keeps its number, by masking rather than by skipping the line.** It points FORWARD, at work that sits in no commit yet, so the card is the only pointer there is and `git log -S` has nothing to find; the comment rule already blessed it. Masking the form means a provenance id sitting beside a `TODO` on the same line is still refused, which skipping the line would have let ride along. Eleven sites; a twelfth, `TODO(HATS-120b)`, was never visible to the pattern at all — `\b` after the digits needs a non-word character and finds a letter — so it is spared without being counted, because a tally has to name sites the gate would otherwise flag.

  **A user-facing message is not the marker's case.** The marker exists for an id a machine prints about itself; a tombstone that tells a consumer *the `plan_sections:` channel was removed (HATS-1160, HATS-1149 decision A)* prints a dead link to someone who has no tracker. Four tests pinned exactly those ids as their sentinel, which is what kept them alive. Each now pins something a reader can act on instead: the retired channel by name, `AI_HATS_LIBRARY_ROOT`, and — where the record mattered — `ADR-0019`, which ships inside the repository and which `adr-integrity` refuses once it stops resolving. Three migration step labels moved with their pinned set in the same edit.

  **The shape that got away twice now has a guard, and the guard has a control.** A parenthetical opening a *wrapped* line leaves a hanging mark on the next one — `#.` in code, `,\n).` in docs — and the docs half of this work shipped two of them because the check was written after the commit. The scanner reads the ADDED side of each hunk against that same hunk's removed side, so the repository's own idioms are not findings, and it carries planted damage it must find plus the false positive it must not: a Sphinx `#:` comment, excluded by an explicit check rather than by a regex that hands its own colon back to a punctuation class — the spelling that cost 15 spurious findings the last time. Zero of six shapes across 1308 added lines. Of 1307 removed lines, exactly two were not prose in the pre-image, both a `ruff format` re-glue of a call whose shortened argument let it fit on one line.

  **What the sweep found that it was not looking for.** A *truncated* id — `pre-865`, `pre-549`, `(479)`, a bare `**814**` — is the same reference with its prefix torn off, invisible to the gate's pattern and therefore to everyone; several were already orphaned by the id they leaned on. A comment in `cli/worktree.py` called a base-branch mismatch *the same wrong-branch class as HATS-486*, and HATS-486 is stale git-lock recovery: a reference dead even **with** the tracker, of the class that turned up five times in the docs. And `src/` still holds 16 ids of this tracker's *other* two prefixes, which the same argument condemns — two of them code literals naming a catalog, so that sweep is a card and not a wider pattern here. The gate says so on every run rather than reporting a clean tree it has not read. `tests/`, `packages/` and `scripts/` stay uncovered for the same reason: judging a tree nobody has swept only reports a backlog. The pin every one of the 316 `tests/e2e/` headers carries is the knot that card inherits — `e2e-catalog` refuses a pin it cannot ground in the file body, and one of them lost its basis here the moment its test stopped asserting an id.
- **A Claude session is read as a stream of typed events, and every token number it produced was wrong** (HATS-1966). The reader treated a JSONL record as an API response. Identity is actually `requestId`, and 75.8% of responses span two to ten records — the CLI splits one call across fragments and repeats that call's `usage` dict byte-identical on each (6166 repeats measured, **0** carrying different values). Summing per record therefore inflated every total: **2.48x** over the 40 largest real transcripts, which is what `usage.json`, `metrics.json` and the audit header have been reporting. Two more defects fell out of the same conflation. `current.response = text` overwrote, so only the last text-bearing fragment of a turn survived — 45.3% of turns lost intermediate prose, **1,256,265 characters** recovered on those 40 transcripts alone. And `tool_result` records were dropped wholesale by the user-text filter, so a failed tool call reached no artifact at all; 314 failures now render on that same sample, against 0 before. Thinking is kept as text rather than `len(text) // 200` printed as seconds. Existing reports are **not** migrated — the schema version says which counting rule produced a file.
- **A run killed by the platform no longer reports success, and its notice is no longer the agent's answer** (HATS-1966). Nothing read an API, quota or auth failure: a grep for those field names over `src/` and `packages/` returned zero, so a session the platform killed was indistinguishable from a short one in every artifact. 91 such records sit in the local corpus (`server_error` 73, `authentication_failed` 13, `rate_limit` 5), and in **15** of them the error prose was the last assistant record — "You've hit your monthly spend limit" rendered as what the agent said. Failures are now typed by who must act rather than by the provider's error vocabulary, so a second surface maps onto them instead of growing its own: a person must re-authenticate or pay, the harness must wait (carrying the reset time the transcript reports) or retry or abort, or the event is only worth recording. A record that produces a blocking signal contributes no text to the transcript. Acting on a signal is deliberately not in this change — `retry_after` is recorded as a fact, and the retry loop still sees only timeouts.
- **The schema-drift detector means something again** (HATS-1966). `usage.py` knew 12 record types against 23 that occur, one of which (`summary`) occurs zero times, so `unknown-entry-types` fired on **59.4%** of sessions and carried no signal. All 23 are classified and an unmodelled record is reported individually rather than lumped; over 700 transcripts the drift signal is now **0**, while a synthetic unknown type still produces exactly one.

- **A mistyped subcommand is refused instead of launched as a prompt** (HATS-1932). Bare `ai-hats` forwards positional text to the provider, which is right for prose and wrong for a command name: `ai-hats githooks --help` reached `claude` as the prompt `githooks --help`, claude parsed `--help` and printed its own usage, and the wrapper SIGTERM'd it once the PTY grace elapsed — `exit_code=143`, `duration_s=6.012`, and a session directory whose only content was the corpse. Five field sessions carry that signature. Being mounted is what makes a name real, so a lone word that resolves to no command is refused by default — a command retired tomorrow cannot silently reopen the defect for its own name, which a hand-kept blocklist could not promise. Prose is what the default lets through, in the two shapes it actually takes: several tokens (`ai-hats hello world`) or one carrying spaces (`ai-hats fix the bug in cli.py`). On top of that default sits detail derived from code rather than guessed from shape: every command path in the click tree, so a nested name typed at the top level gets its real one (`init` -> `ai-hats self init`, `roles` -> `ai-hats list roles`), plus a map that names where a retired capability went, and catches the multi-token invocation (`ai-hats task list`) a lone word cannot — `githooks`, which is a module entry point and never a subcommand, and `task` / `hyp` / `proposal` / `attach` / `run`, retired at the rack cutover and the `agent` rename. Those four had been launching sessions with prompts like "task list" ever since: the groups were unmounted and the `--help` roster pruned, but nothing guarded the top-level swallow that catches whatever the roster no longer lists. Edit distance was measured and rejected as the trigger, not merely passed over — `difflib.get_close_matches("githooks", ...)` returns `[]` against the top-level commands at cutoff 0.6 *and* 0.5, so a did-you-mean gate would have missed the very invocation that prompted the work. Prose passthrough is untouched (`ai-hats hello world`, `ai-hats fix the bug in cli.py`), which is the regression gate the work was steered by: the HATS-1202 test that pins it passes unmodified. The same ownership question decides flags, and there the default has to be the other way round: unknown flags are the provider's by design, so only the ones ai-hats declares on its own root group are refused — read off that group rather than typed out. `--help` had merely been the member of that set anyone noticed. Twelve others leaked, and the loudest was `--dry-run`: because ai-hats' flags must precede the prompt, `ai-hats hello --dry-run` forwarded it and **launched a real session** — the one thing the flag promises not to do. `-p` and `--tag` after a prompt were silently dropped the same way, the override ignored and the text handed to the provider. Each now says that ai-hats' own flags go before the prompt. `--` means the text literally — `ai-hats -- githooks --help` forwards verbatim — and `--help` now says so. Two traps sit in the click seam and are worth naming, because each shipped as a regression in a working draft before a test caught it: click mutates the argv list *in place* while parsing, so the `--` escape has to be read before `super().parse_args()` or it is already gone; and a resolved subcommand keeps its own flags in `ctx.args` while sitting in `_protected_args`, so reading `ctx.args` alone refuses `ai-hats wt --help`. The refusals name a command to type next, and the `ai-hats ...` half of every such pointer is now pinned against the live tree by a test — two of them named `rack hyp ls` and `rack proposal ls`, spellings rack does not have (listing is `rack ls --backlog <name>`), which is the same defect the message exists to prevent.

- **The README lists the CLI groups that exist** (HATS-1932). The table said eight against nine real ones, omitted `execute` and `wait`, credited `self` with a `rollback` removed at HATS-407 and a `clean` no version of the group appears to have carried, and advertised a `task` group unmounted at the rack cutover — sending readers at the exact word the CLI now refuses. A `--tree` example walked into that dead group too.
- **A tracker id in a doc a reader opens is refused** (HATS-1915). `ticket-ids` judged the prose the library ships and nothing else, on the argument that `PROJ-1430` is a dead link in someone else's project. `git ls-files .agent` returns **0**: the tracker is absent from a fresh clone of *this* repository too, so 341 ids across 17 living docs plus `CONTRIBUTING.md` were dead links by the same argument — `docs/glossary.md` alone held 143. All 341 are gone: the sentence now carries the fact, an ADR cited beside an id keeps its place while the id goes, and where the *when* mattered the card's closing date replaced its number (*"Until HATS-1826"* became *"Until 2026-08-26"*). Five were dead links even **with** the tracker — HATS-050, HATS-088, HATS-252, HATS-294 and HATS-341 name no card at all. A **fenced block is a sample, not a claim**: the 57 ids inside one — `rack transition HATS-042 --link depends_on:HATS-041` needs two distinguishable numbers to teach the grammar of a link — are untouched and unjudged, and the section of `CONTRIBUTING.md` documenting the rule failed the gate it documents until its own examples moved inside a fence. `src/` stays out of the corpus: four fifths of its ids sit in docstrings, which is prose of the same class and wants the same sweep rather than a gate over a list of files.

- **A nested code fence no longer un-judges the rest of its file** (HATS-1915). `strip_code_fences` toggled on every line starting with three backticks, so a four-backtick block holding three-backtick ones counted as three toggles instead of two: the parity flipped and everything after it silently left the corpus — of `prose-refs`, and of the extended `ticket-ids` behind it. Fences are now read per CommonMark in `scripts/_markdown.py`, where a run closes only on the same character, at least as long, with nothing after it; an **unclosed** fence is reported by name instead of swallowed. That report earned its place on the first run. `docs/how-to-feedback-loop.md` carried an orphan of an older revision — a heading with no body above a code block whose opening marker was lost — and behind it sat four `library/core/…` paths that HATS-1907's sweep could not reach. The shipped `golang-documentation` README template left a `bash` block open inside an HTML comment, so anyone using it got the License section rendered inside a code block.

- **A fully-proven zone no longer reads as a red one** (HATS-1949). The e2e tier is partitioned into zones that overlap on purpose, and the tier memo deselects a test the same tree already passed under another stage's name; when that covers a whole zone, pytest exits 5 and `pytest_sessionfinish` rewrites it to 0. It counted only the controller's own deselections. Under xdist — which the gate turns on whenever it is installed, so in practice always — collection happens in the workers and the exit code is judged by the controller, so the count was zero and the 5 stood: `e2e-rack`, whose every test is also claimed by `e2e-gates`, `e2e-wt` or `e2e-guards`, failed the gate for having nothing left to run. The workers now send their counts home on `workeroutput` and the controller sums them as each shuts down. The defect could only surface once every other zone was green, which is why it sat behind six stale tests; the serial path was always correct, and the existing test exercised only that, so the new one runs the same case under `-n 2`.

- **The e2e tier is green on master again, after seven reds stacked behind one another** (HATS-1949). The tier stops at its first failing zone, so each fix revealed the next and `E2E_RC=1` refused every merge and every push for a reason belonging to no branch. Six were stale tests. `test_done_gate` asserted the maintainer role's config contains `ai-hats-maintainer` — a trait it composed until that single-composer trait was folded into the role, after which the assert matched a dead indirection; the layer path and the injection's subject already carry what the test is about. The hand-kept `GUARDED` registry in `test_hook_chain_fail_open_recorded` had missed `wt_git_gate.py` since it was armed on the Bash chain, so the newcomer's fail-open was never exercised — exactly the drift the registry's completeness check exists to catch. The remaining four share one shape and it is worth naming: **code that lives inside a string and runs in a child process**. Three inline `-c` snippets and one out-of-tree provider stub still passed a bare `Path` where the layout refactor made `ProjectLayout` the argument, or dropped the cache root the update check now requires; no import, type hint or linter in the parent process can see into a string, so a signature change sweeps the repository and leaves them behind — green until something runs them. Two codex e2e also pointed `CODEX_HOME` at a directory they never created, which the surface refuses.
...
- **A change runs the tests its own area owns, at `->merge`** (HATS-1921). Until now no card gate ran a single e2e: the tier stood on `push-gate`, after everything was already merged, and in CI after the fact. Measured over the last sixty first-parent merges, 29 touched the trigger surface and every one of them entered master without one. The tier now splits into **zones** — an area of the codebase together with the tests that assert it, one pytest marker plus the path prefix that owns them, one row of `gates.sh zones`. Each zone is a named stage of its own, so a card gate requires what it declares PLUS what `gates.sh touched` reads out of the diff, and `e2e-rack` appears in a refusal that no gate's `--stages` prints. The half no zone claims is `e2e-default`, and it stands on `->done`, where a supervisor is present for the breakage two independently green branches make together (ADR-0023 D3, D11). **The marker model is untouched, which is the point**: the first design assumed a diff-dependent requirement forces the marker to carry its own set, but a zone stage is *named* and its selection *fixed*, so markers stay per stage per tree and only the requirement moves with the diff — a base branch that advances and narrows the diff leaves old markers valid, one that widens it names a stage with no marker and gets an ordinary refusal. `push-gate` swaps `e2e` for the parts so a tree never runs the same tests twice under two names, and `tests/test_e2e_zone_partition.py` holds the parts equal to the whole by asking the stages themselves through `--collect-only` — a copy of their marker expressions in the test would be the very defect it exists to prevent, the tier's selection having already lived in two copies with no test between them. What no machine can check is that a zone is honest, so the rule is: **zone a test by what it asserts, never by what it drives on the way.** That one cost a pass on the way in — tagging the codex consent cohort because it invokes `rack` put 36 tests of another surface into the rack zone, 35 of them red for reasons rack never touched, and by that rule 68 of the tier's 307 files would have qualified. The default bucket is meant to shrink to nothing as areas get marked, and the price of that is said out loud in the ADR: once it is empty the card gates stop catching the unexpected regression, which stays where it is today, on `push-gate` and in CI. The floor under `->done` is `merge-smoke`, which does not move. Measured 2026-09-09: the full tier 295 s at `-n 8` (up ~18% in three weeks), the rack zone 26 tests in 27.7 s. A tree whose `gates.sh` predates the verb has no zone stages to earn either and is told so rather than refused — the alternative blocks every card already in flight.
