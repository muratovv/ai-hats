...
- **The smoke subset is asked before the merge, not after it** (HATS-1673). `unit` and `integration` never touch the installed binary; `merge-smoke`, the curated subset of `tests/e2e`, is the one stage that checks ai-hats still launches, and it stood on `->done` alone, after the branch was already in master. It now sits on `->review` and `->merge` as well, and the two keep demanding the same set, so one run still clears both; `make done-gate` after a green `review-gate` pays for `integration` and `e2e-default` only. Measured on a clean tree under xdist (`-n8`, 14 cores): `merge-smoke` 6.6 s against `unit` 62 s, and the whole `review-gate` from scratch, with the zones this diff itself touches (`gates`, `rack`), 183 s.
