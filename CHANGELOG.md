...
- **Every tool call paid 105 ms to import a dispatcher** (HATS-1869). The hook dispatcher is a fresh process on each tool call, on agy, cline, codex and opencode alike — including opencode's hookless roles, where it spawns and finds no rows. Three package `__init__` files on its path each pulled the same heavy subgraph: `ai_hats.surfaces` and `ai_hats.surfaces.<surface>` reached the schema layer through the surface contract, and `ai_hats_core` cost pydantic, filelock and asyncio to reach `Deadline`, which imports nothing but stdlib. Measured, not inferred, and the reason the obvious fix was not enough: the three edges are independent, so cutting any two of them saved nothing at all — only all three together moved the number, from ~105 ms of import to ~20 ms (wall ~118 ms → ~34 ms per dispatcher, same interpreter, back to back). All six facades now bind their exports lazily (PEP 562); every name resolves as before — the deprecated `Provider` aliases, and the submodules a bare `import pkg` used to expose only because the facade had imported them. Because one eager import anywhere on that path silently restores the whole cost, `tests/test_dispatcher_import_closure.py` refuses a dispatcher whose import closure contains any of them.
