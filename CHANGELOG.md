...
- **A local channel with no source is healed by `self update`, not handed to uv** (HATS-1998). `harness: {channel: local}` without a `path` means "editable install of the project root", which is a Python project only for the ai-hats checkout itself; the wizard writes exactly that when `local` is chosen on a host whose ai-hats is a git install (no editable source to detect), and every heal contour looked past it — the launcher heals only a broken venv, `_read_harness` degrades only an unparseable yaml, and the triage had no harness row — so `self update` ran `uv pip install -e <project root>` and printed uv's refusal under an all-green triage. The local source is now resolved once (explicit `path`, else the editable install the process runs from, else the root) and judged by uv's own rule (`pyproject.toml` or `setup.py`); triage carries a `harness` row (`--check` exits 1 on it), a source that cannot install degrades that run to edge with a warning naming `ai-hats config set --channel local --path <checkout>`, the yaml stays the user's, and the launcher's recreate falls back to the pip target the same way. `_run_editable_update` takes the resolved absolute path; the cwd-relative `.` is gone. Found alongside: the launcher's foreign-pin guard (HATS-944) re-pinned `AI_HATS_PROJECT_DIR` and `AI_HATS_VENV` to the cwd project but left the session envelope, and the python entry resolves the project from the envelope (ADR-0024), so a command run from another project inside an agent session still answered for the session's project — the launcher now drops `AI_HATS_SESSION_IDENTITY`, `AI_HATS_SESSION_ID` and `AI_HATS_ROLE` with the pin. A direct `python -m ai_hats` from a foreign cwd keeps envelope-wins by design.
