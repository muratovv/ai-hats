...
- **A gate is a row, not a file; a marker is per stage, not per gate** (HATS-1878). The third gate on the checks channel had cost a fourth near-identical script, edits to three hand-kept name lists inside `scripts/ci-local.sh`, a Makefile target, a role row and an ADR row — and the ADR row rotted: seven of the twenty-three stages the dispatcher knew were named nowhere in ADR-0023. The mechanism is now three layers, each forbidden to know the next one's concern. `scripts/ci-local.sh` runs a **stage** and knows nothing else; a stage runs bare — no argv reaches pytest, since a marker for `unit -k foo` would be a lie, and CI's parallelism rides `PYTEST_ADDOPTS`. `scripts/ci-gate.sh`, the **primitive**, takes a list of stages: `check` says which lack a marker for the subject tree and has no code path to the runner, `run` runs only those, stops at the first red and stamps each green one. `scripts/gates.sh` is the one place a **gate name** appears in code: a table whose rows are stages and whose columns say which gates require each, in run order. Where a gate applies is the role's `composition.apps` row alone — `run: maintainer-quality-gate/hooks/gate.sh` plus `gate: <name>`, which the engine now forwards to the hook as `AI_HATS_CARGO_GATE` (a row's own keys never reached the script it declared). One `hooks/gate.sh` replaces `review-gate.sh`, `merge-gate.sh` and `done-gate.sh`; the library names no stage and no gate. Markers live at `<git-common-dir>/ai-hats/stages/<tree>/<stage>`, so `make done-gate` after a green `make review-gate` pays for `master-ci`, `integration` and `merge-smoke` and nothing else, a red stage keeps the stamps earned before it, and a refusal names exactly the stages missing. The subject is always a commit: a run happens in the checkout only when it is clean and at that commit, otherwise in a one-shot scratch checkout — a dirty desk taints no marker and blocks none, and neighbours merging into master cannot move what is being judged. The old marker format is not read; each live card re-earns once. ADR-0023 is rewritten under the three layers and its two inventory tables render from `scripts/gates.sh` and the bindings, held current by the new `gate-table` stage. Found on the way: `tests/e2e/test_done_gate.py` had been red on master since the review gate landed — its fixture walked the hand-off with no marker, and no card gate runs that file.
