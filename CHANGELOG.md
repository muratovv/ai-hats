...
- **A `tech-writer` role, for editing prose rather than writing it** (HATS-1890). Its default move on any text — the user's draft or its own — is a critique, not a rewrite: name the one thing the text is for, name what is said twice, check that every section hands the reader something. Three skills carry it, and the two reusable ones are context-free by contract so another project can compose them by name: `prose-review` (the report's shape and the per-language style references, Russian and English), `claim-evidence-check` (every checkable statement becomes a row with a source or an honest `unverified` / `contradicted` mark, plus a stdlib link checker), and `article-outline` (the Diátaxis quadrant, audience and style forks settled before a word of body text). Portability is held by a grep with a positive control: the same pattern that finds nothing in the two reusable skills finds `plan.md` in `article-outline`, where the ai-hats glue deliberately lives. What the role knows about editing is deliberately not in the always-on injection — conduct rules with worked examples sit in `prose-review/references/editor-conduct.md`, and the FSM is left to `hatrack` and `rule_backlog_discipline` rather than restated. The role reviewed the prose that defines it before shipping, and its own findings are folded in: a style reference that filed деепричастия under причастия, a link checker that reported `checked 0 link(s)` over a corpus whose one citation it could not see, and a conduct example that marked a claim `unverified` while citing the source contradicting it.
