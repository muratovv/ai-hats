...
**Auto session-retro.** После завершения сессии хук `session_end_auto-retro.sh` автоматически решает, нужно ли генерировать retro. Поведение настраивается через `profile.json` → `feedback.session_retro`:
...
profile.json                           # Активная роль + feedback config
